...
- 実装状況（2026-09-20追記。D-181・D-183の当時の本文と成立範囲は変更しない）。
  ...
  - RenderFragmentごとの配置（d5334f8）に残るもの：
    ...
    - **Cut DAGは、台帳の採用面を切断Workへ渡すときに登録のframe写像を適用していない**ため、Temporary表示が切る面と実Geometryが切られる面の一致が保証されない。今回の入力（`lineageToGeometryLocal`が`Translate(0,−0.4,0)`、採用面が`y = 0`）では不一致を確認した。今回の描画確認は、ハーネス側で面を変換して実切断を行い、製品の`TryCommitCut`を通したものである。
...
Phase 3のうち、非同期切断Work（773bb09）、Cut DAGの骨組み（c97e43c）、実Geometry Commitと表示差替え（0df0392）の3単位は、2026-09-20に区切った（4.5.6の「実装状況」）。確認は静的Geometryと合成Final Physicsによるもので、Final Physicsのこの表示経路への接続（直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」による）、動く子の配置の受渡し（表示基盤は5.6の「実装状況」の範囲で実装済み、Physicsからの接続は未実施）、Character、製品の構成根、この経路でのPlayer・性能は未接続または未確認である。Phase 3全体とT-006・T-083・T-074のPhase 3部分は完了扱いにしない。

Phase 4のうち、Cut／Cookの未公開成果物まで（74bbd8e）、正負Final Owner候補の未公開構築まで（2135bf4）、直接Final分裂の物理・論理とOwner対応の公開接続（cc168a6）の3単位は、2026-09-20に区切った（7.2の「実装状況」）。確認は合成入力によるもので、Temporary表示の追従のPhysics側からの接続（表示基盤はd5334f8で実装済み。5.6の「実装状況」）、動く子の配置の受渡し、Hit／Queryの解決、Provisional・D6・Final handoff、Cut DAG接続、Character、製品の構成根、Player・性能は未接続または未確認である。7.1.2全体の一体公開は未完成であり、Phase 4全体と関連する受入れ項目は完了扱いにしない。
