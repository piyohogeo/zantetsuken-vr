...
未接続・未確認（この単位では扱わない）。Final Physicsの公開と置換がこの表示経路へ接続されていないこと（直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」の範囲で成立しており、公開後もGeometry Commitまで描かれるのはSourceの登録である）、動く子の配置をPhysicsから表示へ渡す接続、Characterのskinningと骨に付いたConvexとの接続、製品全体の構成根への接続、この経路でのPlayerと性能、統合後の追加検証。これらが残ることは、過去に別の条件で確認済みの描画条件を未確認へ戻す意味ではなく、過去のXR証拠を今回のCommit経路の証拠に流用する根拠にもならない。Phase 3全体および関連する受入れ項目の完了は意味しない。
...
- 実装状況（2026-09-20追記。D-181・D-183の当時の本文と成立範囲は変更しない）。
  ...
  - 未解決：O-034（製品の`MaxStencilColors`など。試験値8は製品値ではない）。実Geometry Commitとの接続は4.5.6の「実装状況」の範囲で成立しており、そこに残る未接続・未確認（Final Physicsのこの表示経路への接続。直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」による。動く子の配置の受渡しとTemporary表示の追従、Character、構成根、Player・性能）が後続となる。
...
実装状況（2026-09-20追記。上の目標仕様と既存の決定本文は変更しない）。物理切断の数値処理・cook、その成果物から作る正負Owner候補の構築、そして直接Final分裂についての物理・論理とFragment→Owner対応の公開接続を3単位で実装し、いずれもlocal mainへ統合した。詳細はここにまとめ、関係する節からは本項を参照する。表示の追従は含まないため、7.1.2全体の一体公開は未完成である。
...
Phase 3のうち、非同期切断Work（773bb09）、Cut DAGの骨組み（c97e43c）、実Geometry Commitと表示差替え（0df0392）の3単位は、2026-09-20に区切った（4.5.6の「実装状況」）。確認は静的Geometryと合成Final Physicsによるもので、Final Physicsのこの表示経路への接続（直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」による）、動く子の配置の受渡しとTemporary表示の追従、Character、製品の構成根、この経路でのPlayer・性能は未接続または未確認である。Phase 3全体とT-006・T-083・T-074のPhase 3部分は完了扱いにしない。

Phase 4のうち、Cut／Cookの未公開成果物まで（74bbd8e）、正負Final Owner候補の未公開構築まで（2135bf4）、直接Final分裂の物理・論理とOwner対応の公開接続（cc168a6）の3単位は、2026-09-20に区切った（7.2の「実装状況」）。確認は合成入力によるもので、Temporary表示の追従、動く子の配置の受渡し、Hit／Queryの解決、Provisional・D6・Final handoff、Cut DAG接続、Character、製品の構成根、Player・性能は未接続または未確認である。7.1.2全体の一体公開は未完成であり、Phase 4全体と関連する受入れ項目は完了扱いにしない。
