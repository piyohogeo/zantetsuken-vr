...
退化面の属性はfiniteに保つが、定義できない幾何法線の正規化を要求しない。アセット前処理とPhysics Convexの非退化条件は変更せず、スキニング後または切断後のRuntime表示Geometryへ再適用しない。Fixture固有の検証は17章の実装詳細とする。
...
表示の実行時表現は4.5に従い、読み込み時Unity Meshから切断時VPへ移行する。Component単位のGeometry参照を保持し、複数参照による表示構成と論理・物理所属を分離する。Component識別用参照と描画用Descriptorを区別し、実切断結果は4.5.6の正負の共通Index配置へ仕上げる。Fixture保存形式はRuntime AoS表現と独立した実装詳細であり、VP導入だけで形式改訂や一括再生成を要求しない。
...
| D-169 | 基本Playable先行Phase | 0.55でUX、4.50～4.52でWaveと現在状態切断を先行完成し、Predictionを後段へ分ける。4.1は性能曲線、Slash Deadlineへの適用は4.53とする | 人間承認済み、2026-09-11。15章の依存・省略条件を正本とし、4.55／4.60の内部方式は変更しない。Traceの現行相関は21.16.6に従い、旧形式の扱いは17章に従う |
...
T-090では入力`EvaluationWorkItem`にSequence fieldが存在しないこと、Descriptor不正／CapacityExceeded／NotAcceptingが次の成功受付のSequenceを進めないこと、受付成功だけが連続Sequenceを内部Recordへ割り当てることを検査する。不透明`WorkToken`からSequenceのbit layoutを推測せず、`TryGetState`の診断SnapshotとTask lifecycle TraceのEnqueueSequenceが同じ内部値を返すこと、古いToken世代が再利用SlotのSequenceへアクセスできないことも確認する。
...
| Phase 0.14 | 可変長Trace保存・読込みと切替 | 21.16.4のboundedな保存・読込みを接続する | 記録の相関と不完全性を維持し、全record配列を作らず保存・読込みできる。成立後にRelease既定を切り替え、置換済み旧経路を削除できる。形式・旧Reader・Goldenの維持は要求しない |
...
> **次の推奨アクション** Phase 0／0.1の観測能力を利用し、進行中の0.11を21.15の範囲で完了する。0.2は採用Fixtureをmergeして閉じる。15章の独立系列に沿ってSlash Sandboxと表示系列を進め、基本Playableを優先する。OpenXR API Layerと連続録画はPhase 4.8で必要性を判断する。
...
対象はDomain Trace、Writer、History、保存／読込み、およびCapture終了時のproducer停止・最終Drain・sealとの接続とする。Encoderや映像取得経路を変更しない。0.12～0.14を同一変更系列の内部checkpointとし、0.14成立時にRelease既定を一度切り替える。旧新backendの並行搭載・二重記録を要求せず、置換済みのWriterと専用試験は同系列で削除できる。旧保存形式のLoader・Golden・再exportを維持する義務はなく、17章の扱いに従う。
...
0.12は21.16.2のWriter公開順・所有権・非待機・bounded Drain、0.13は21.16.3の容量とcommitted prefix・停止後View、0.14は21.16.4の保存・読込みと切替を確認する。代表負荷で記録・回収・保存の費用、allocation、欠落とメモリを確認する。具体的なFixture、形式の境界値、試験手順は実装詳細とし、旧形式互換試験や巨大な条件直積を完了条件にしない。
