...
| Phase 1.50 | 選択済みVP経路のXR／Single Pass確認 | 選択済みVP経路（D-178によりStage 3。0.94の採用記録はStage 2）、既存Geometry参照・Instance Transform・Draw DescriptorのColor／Depth描画 | 0.5とPhase 1の完了後、Quest Linkで一つのGeometryを複数Transformで表示し、左右眼のGeometry／Transform／Instance選択、片眼欠落がないことを確認する。Clip・Stencil・Shadow品質・Stage再比較・製品90fps SLAは含めない |
...
Phase 1.50～1.52は今回の文書更新では閉じない。Stage 3経路での先行確認（非XR、XR Simulator、Quest Link）は根拠として残すが、いずれもMSAA無効の構成で行ったものであり、**採用MSAA構成が決まったことを意味しない。採用MSAA構成は未決定のまま残し、値を推定・既定しない。** 各Phaseの完了は各行の条件に従って別途判断する。1.51で要求する採用MSAA構成は未決定であり、既存のMSAA無効での確認を採用決定とは扱わない。
