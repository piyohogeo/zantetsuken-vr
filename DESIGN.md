...
未接続・未確認（この単位では扱わない）。Final Physicsの公開と置換がこの表示経路へ接続されていないこと（直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」の範囲で成立しており、接続がないため公開後もGeometry Commitまで描かれるのはSourceの登録である）、動く子の配置をPhysicsから表示へ渡す接続（RenderFragmentごとの配置を受け取る表示基盤は5.6の「実装状況」の範囲で実装済みで、そこへOwnerの位置を渡す製品接続が未実施である）、Characterのskinningと骨に付いたConvexとの接続、製品全体の構成根への接続、この経路でのPlayerと性能、統合後の追加検証（frame写像の接続についても、統合後に追加の検証は行っていない）。これらが残ることは、過去に別の条件で確認済みの描画条件を未確認へ戻す意味ではなく、過去のXR証拠を今回のCommit経路の証拠に流用する根拠にもならない。Phase 3全体および関連する受入れ項目の完了は意味しない。
...
- 実装状況（2026-09-20追記。D-181・D-183の当時の本文と成立範囲は変更しない）。
  ...
  - 未解決：O-034（製品の`MaxStencilColors`など。試験値8は製品値ではない）。実Geometry Commitとの接続は4.5.6の「実装状況」の範囲で成立しており、そこに残る未接続・未確認（Final Physicsのこの表示経路への接続。直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」による。動く子の配置の受渡し、Character、構成根、Player・性能）が後続となる。
  - RenderFragmentごとの配置（d5334f8）に残るもの：
    - **Physics Owner対応・公開処理からこの配置照会への製品接続は未実施**である。表示側は配置を受け取れるようになっただけで、Ownerの位置を表示へ伝えるものはまだ無い。表示基盤の完成を、物理公開からの追従の完成とは扱わない。
    - **Ignored集約の追従方式は未決**である。集約を解くと、Ignored面で切っていない同じ形状が枝の数だけ別の場所に描かれる。単一配置で集約を維持すると、集約形状がそれが表すどの剛体からも離れる。どちらも品質例外の追加を要し、**今回はどちらも追加していない**。集約RenderFragmentをどう扱うかは照会側が明示的に答える形にしてある。
...
実装状況（2026-09-20追記。上の目標仕様と既存の決定本文は変更しない）。物理切断の数値処理・cook、その成果物から作る正負Owner候補の構築、そして直接Final分裂についての物理・論理とFragment→Owner対応の公開接続を3単位で実装し、いずれもlocal mainへ統合した。詳細はここにまとめ、関係する節からは本項を参照する。表示の追従は含まない——RenderFragmentごとの配置を受け取る表示基盤は5.6の「実装状況」の範囲で実装済みだが、ここからその照会へOwnerの位置を渡す製品接続は未実施である——ため、7.1.2全体の一体公開は未完成である。
...
- **物理・論理とOwner対応の公開接続（cc168a6）。** 直接Final分裂について、上の候補を公開する。生存Fragmentがどの物理Ownerかという対応を、台帳が既に持つFragmentを鍵として保持し、Fragmentの再切断入力に使う対応を用意した。Hit／Queryの解決からこの対応への接続は未実装である。公開する対象のFragmentは当該Operationの記録から取り、呼出側が指定した場合は一致を要求する。切り出した入力Shapeが現在のOwnerのものであることも照合し、受付後に別Ownerへ差し替わった成果物は適用しない。判定順序は、台帳の既存判定（生存性・Active Operation・authority・Anchor準備と子2件分の領域確保）を先に通し、その後で現在Owner／Shapeの照合へ進む。台帳の判定は台帳内で共用し、外側へ複製しない。
  公開直前にSourceの現在の姿勢・COM線速度・角速度を読み、候補の配置と初回分裂の速度をそこから作り直す（質量・重心・慣性は数値局所frameの量なので変えない。Anchorを持つ側は固定のままで速度・Offsetを与えない）。通常の移動はStaleではない。分離Impulseは直接Final経路で一度だけ、自由な側にのみ、採用面法線の各側方向へ加える（値は呼出側が渡すもので、製品値は本節では定めない）。続いて同じMain更新区間で、候補の有効化と質量特性等の適用、台帳による論理公開、生成Meshの所有権移転、子2件のOwner登録、Sourceの物理対象の退役を行う。途中に物理Step、新規受付、Query解決、Renderer収集を挟まない。表示への切替は行わない。
...
未接続・未確認（これら3単位では扱わない）。**Temporary表示の追従の、Physics側からの接続**。RenderFragmentごとに配置を受け取る表示基盤は5.6の「実装状況」の範囲で実装済みだが、**Owner対応・公開処理からその照会へ位置を渡す製品接続は未実施**であり、公開後もGeometry Commitまで描かれるのはSourceの表示登録のままである。したがって**7.1.2全体の一体公開は未完成**である。Cut DAG（4.5.6）のGeometry経路への接続のうち、採用面のframe写像は4.5.6の「実装状況」の範囲で成立した（b98661b）。この公開経路からGeometry側を駆動する接続は残る。Hit／Queryの解決自体（対応を引く先は用意したが、解決処理は書いていない）。Provisionalと必要なD6、Provisional→Final handoff。今回の接続は直接Final経路のものであり、これらの完成でも、通常切断を旧物理のままFinal待ちへ変える仕様変更でもない。Characterのskinningと骨に付いたConvex、製品全体の構成根、この経路でのPlayerと性能、統合後の追加検証。これらが残ることは、過去に別の条件で確認済みの項目を未確認へ戻す意味ではなく、今回の証拠をそれらへ流用する根拠にもならない。Phase 4全体および関連する受入れ項目の完了は意味しない。
...
Phase 3のうち、非同期切断Work（773bb09）、Cut DAGの骨組み（c97e43c）、実Geometry Commitと表示差替え（0df0392）、採用面のframe写像の切断入力への接続（b98661b）の4単位は、2026-09-20に区切った（4.5.6の「実装状況」）。確認は静的Geometryと合成Final Physicsによるもので、Final Physicsのこの表示経路への接続（直接Final分裂の物理・論理・Owner対応の公開までは7.2の「実装状況」による）、動く子の配置の受渡し（表示基盤は5.6の「実装状況」の範囲で実装済み、Physicsからの接続は未実施）、Character、製品の構成根、この経路でのPlayer・性能は未接続または未確認である。Phase 3全体とT-006・T-083・T-074のPhase 3部分は完了扱いにしない。

Phase 4のうち、Cut／Cookの未公開成果物まで（74bbd8e）、正負Final Owner候補の未公開構築まで（2135bf4）、直接Final分裂の物理・論理とOwner対応の公開接続（cc168a6）の3単位は、2026-09-20に区切った（7.2の「実装状況」）。確認は合成入力によるもので、Temporary表示の追従のPhysics側からの接続（表示基盤はd5334f8で実装済み。5.6の「実装状況」）、動く子の配置の受渡し、Hit／Queryの解決、Provisional・D6・Final handoff、この公開経路からGeometry側を駆動する接続（採用面のframe写像はb98661bで成立。4.5.6の「実装状況」）、Character、製品の構成根、Player・性能は未接続または未確認である。7.1.2全体の一体公開は未完成であり、Phase 4全体と関連する受入れ項目は完了扱いにしない。
