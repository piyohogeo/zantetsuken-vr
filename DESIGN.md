...
共通 Publication の正本経路は `CapturePublicationPlanBuilder -> ICapturePublicationPlanStore.WritePlan` とし、Recovery は `ICapturePublicationPlanStore.ReadPlan` から bounded canonical deserialize した汎用 `CapturePublicationPlan` を入力に再開する。読み取りは固定 byte 上限 + 1 で停止し、canonical 再 serialize と完全一致しない文書を拒否する。旧 `PngJsonCapturePublicationPlan` 系は既存 Phase 0 ファイルとの互換専用であり、新規共通 Publication／Recovery の正本にはしない。
