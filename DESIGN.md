...
| Phase 0.11 | 最小NVENC bounded chunk Backend | D-137～D-139の`NvencBringUpProfileV1`とWindows／NVIDIA／D3D11固定の`NvencCaptureEvidenceBackend`をPhase 0.1の非同期境界へ追加する。指定hardware-qualified環境のnominalだけを30fps／120 tick／4秒提出窓／30秒Finalizationとし、一般CIのlifecycle試験とfault系は短い決定論的caseまたは最大16 tick／1秒提出窓／10秒Finalizationとする。GPU TextureからNV12変換してNVENCへ渡し、PNG用RGBA CPU readbackを経由しない。Main／Render ThreadでGPU／NVENC完了、bitstream、hash、I/Oを待たず、各Frameのraw Annex B Access Unitをaccepted順に1つのbounded Run chunkへappendする | hardware-qualified nominalは120件すべてのAccepted／Frame Completion、1件の確定chunk Artifact、120件のFrame Relation、`CaptureRunArtifactCompletion`、Publication、CaptureCompleteを要求する。H.264 High、IDR-only、SPS／PPS反復、P／B FrameとFrame間参照なし、CQP 28を要求設定として記録し、raw bytesを変更しない。正常Drain／Join後にchunk単位でhash、close、rename、Run Artifact Completionを各1回行い、`Flush(true)`は要求しない。0 Frameまたは未確定chunkは全体を破棄してよい。clean Decoder process 1回で確定chunkから120 Frameを順次decodeし、基本寸法／orientation／色を確認する。Render callbackはboundedな登録だけで戻り、固定Surface／Work／chunk容量枯渇時はBackpressureまたはCapture失敗終端となる。このbounded chunk形式を連続運用へ昇格しない |
...
Submission の所有権は線形とする。`Accepted` のときだけ Surface 所有権を backend へ移し、有効な Work Token を返す。`Backpressured` または `NotAccepting`、および例外では caller が同じ Surface を所有したままとする。Backendは固定容量のWork領域とCompletion領域を使い、stale／foreign／duplicate tokenを拒否する。Completion領域は既存PNG Backendの固定配列Queue、NVENC BackendのWork Slot内inline CellなどBackend内部表現を選べるが、`TrySubmit`のAccepted線形化点より前にFrame Completion 1件と`MaximumArtifactCountPerSubmission`件を欠落なく公開できる容量および必要なBackend資源を一括確認・予約する。全資源を同じtokenへ束縛できた場合だけ`Accepted`を返し、いずれかが不足する場合は内部予約を一切残さずSurface所有権も受け取らず`Backpressured`または`NotAccepting`を返す。Accepted後のCompletion公開は容量不足で失敗せず、受理したtokenごとに`CaptureFrameCompletion`をexactly onceで先に通知し、その`ProducedArtifactCount`に対応するsubmission-scoped `CaptureArtifactCompletion`を各Artifactにつきexactly onceで通知する。複数Frameを共有するRun／chunk Artifactは後述の固定容量Run Artifact拡張で別に終端し、任意のFrame Work Tokenへ偽装してはならない。Source SurfaceのBackend内部解放時点はFrame処理の終端結果と期待submission Artifact数を公開する`CaptureFrameCompletion`とは別概念である。Frame completion、submission artifact completion、run artifact outcome、Run publication completion は別概念であり、単一completionへ混在させない。
...
複数Frame共有Artifactを生成するBackendだけは、Phase 0完了後の破壊的API変更としてV1の`ICaptureRunArtifactProducer`を併設できる。V1は`MaximumRunArtifactCount == 1`だけを受理し、0または2以上、未設定、Run中変更をFrame受付開始前にRejectしてToken、予約、writerを一切生成しない。複数chunk／複数Run Artifact対応はV1へ先取りせずPhase 4.8へ送る。共通CoordinatorはFrame受付開始前にRun専用Registryのatomic factory `TryCreateRunArtifactReservation(TestRunId, ProducerInstanceIdentity)`を厳密に1回呼び、成功時にopaqueな`CaptureRunArtifactToken`と対応`CaptureRunArtifactReservationReceipt`を1組だけ同時発行させる。factoryは未結合receiptまたは後付けTokenを発行せず、receiptへTestRunId、正確なRun Token、Producer instance identity、非再利用generationをimmutableに結合する。Phase 0.11 NVENCは`MaximumArtifactCountPerSubmission=0`、`MaximumRunArtifactCount=1`とする。Run Artifactはper-frame `CaptureFrameWorkToken`ではなく、この同時発行されたRun Tokenへ結合し、全Frame Relationを意味上の関連として保持する。factory失敗では予約もTokenも発行せずBackendを開始しない。Backend初期化が同時発行後に失敗した場合は、同じCoordinatorだけがreceiptとTokenを指定して予約をexactly once解放する。別Backend／別Run／別Tokenへの置換、結合後の再結合、receiptまたはTokenの単独再発行を原子的にRejectする。

`CaptureRunArtifactCompletion`をFrame-scoped `CaptureArtifactCompletion`とは別の固定型として追加する。propertyは有効な`CaptureRunArtifactToken`、TestRunId、`CaptureArtifactDescriptor`、`CaptureArtifactFrameRelation`、後述の`CaptureRunArtifactStagingFinalizationReceipt`とし、`CaptureFrameWorkToken`または単一CaptureFrameIdを要求・保持しない。Run Artifact producerは固定容量領域からexactly onceで`Finalized`または`Abandoned`を公開する。`Finalized`では`TryRegisterRunArtifact(reservationReceipt, CaptureRunArtifactCompletion)`が同一TestRunId／Run Token／Producer identity、未消費予約、Descriptor、Relation、未消費Staging Finalization Receiptを検証して予約とStaging Receiptをexactly once消費し、通常のPublication PlanへDescriptorとRelationを1件登録する。`Abandoned`では`ReleaseAbandonedRunArtifact(reservationReceipt, CaptureRunArtifactToken)`が予約をexactly once解放し、Completion、Descriptor、Plan Entryを生成しない。両APIはFrame Token、foreign／stale Run Token、別Run／別Producer receipt、二重登録／二重解放、登録後解放を原子的にRejectする。既存Frame-scoped Registry予約・`CaptureArtifactCompletion`へRun Artifactを登録したり、120 FrameのいずれかのTokenをprovenanceとして流用したりしない。別のpartial-recovery状態を追加しない。
...
永続成果物の正本は `CaptureArtifactDescriptor` とする。Descriptor は ArtifactId、append-only の ArtifactKind、FormatId/Version、staging/final relative path、ByteLength、ContentHash を持つ。Artifact completion の Work Token は生成処理の provenance であり、Frame と Artifact の意味上の関係とは分離する。関係は独立した `CaptureArtifactFrameRelation` で表し、空集合を Run scoped、複数 ID を複数 Frame 共有 Artifact とする。`CapturePublicationPlan` は Run 相関、Descriptor 集合、この多対多関係だけを canonical に保持し、`.png`/`.json` 固定 field や「1 frame = 1 encoded file」を要求しない。Phase 0 では FrameImage と FrameMetadata の 2 Artifact を各 Frame に関連付けるが、別 backend は 0/1/複数 Artifact、複数 Frame segment、Run scoped Artifact を生成できる。

共通 Publication の Run lifecycle 正本は `CaptureEvidenceRunPublicationCoordinator` とする。既存 `CaptureFrameFreezeTerminalCoordinator.TryCompleteEvidenceRun` が受付停止、queued cancellation、全Frame／submission Artifact Completion反映、backend join、`CaptureRunArtifactCompletion`またはAbandoned解放の反映、未処理slotなし、Frame／Run Artifact予約ゼロ、Trace `Frozen`、同一Runの `CaptureRunInitializationSession` によるOS lock保持を確認した後だけ `CaptureEvidenceRunFreezeReceipt` を発行する。`IsFullyDrained`もqueued cancellation、Run Artifact予約と未確定chunk所有権を含めて再判定する。Plan永続化はこの有効なReceiptを必須とし、Receiptと同じ `CaptureRunRootLayout` を保持する単一 `CaptureArtifactFileStore` をPlan Store兼Artifact Storeとして選択するため、Run AのPlanとRun BのArtifact Storeを組み合わせられない。再起動時は既存 `CaptureRunInitializationOpenOutcome` が `PublicationRecoveryRequired` とOS lock保持を証明し、同一RootLayoutである場合だけ同じStoreの `ReadOrRecoverPlan` から汎用 `CapturePublicationPlan` を復元する。CoordinatorはSnapshotのPlan TestRunIdがOutcomeとStoreのTestRunIdの両方に一致することを確認して `CaptureEvidenceRunRecoveryInspectionReceipt` を発行し、Recovery継続は同じCoordinator発行の有効なReceiptだけを受理するため、別Run Snapshotを差し替えられない。`publication.plan.tmp` だけが残る場合は固定上限内のcanonical文書でRun相関も一致するときだけ非上書きで `publication.plan` へ昇格する。両方存在、別Run、上限超過はcollisionとして変更せず停止する。tmp不正は変更せず報告し、lock下の明示的な `DiscardInvalidTemporaryPlan` だけが再検証後に正確なtmpを破棄できる。読み取りは固定 byte 上限 + 1 で停止し、JSON object化前の無割当preflightでArtifact数、Frame Evidence数、1 Frame当たりArtifact参照数、各文字列長を検査し、canonical再serializeと完全一致しない文書を拒否する。旧 `PngJsonCapturePublicationPlan` 系は既存 Phase 0 ファイルとの互換専用であり、新規共通 Publication／Recovery の正本にはしない。
...
Phase 0.11だけは既存`ICaptureArtifactStore.WriteStaging`を呼ばず、同じ`CaptureRunInitializationSession`、OS lock、`CaptureRunRootLayout`およびpath安全規則へ結合された固定容量`INvencChunkStreamingStagingWriter`を使用する。writerはatomic factoryが発行した正確なTestRunId、Producer instance identity、`CaptureRunArtifactToken`をconstructor時にimmutableに保持し、許可された単一`.partial` pathだけを非共有handleでopenする。append中はchecked ByteLength、streaming content hash、accepted CaptureFrameId列をwriter自身が更新する。`TryFinalize`はappend停止、最小確定条件、Descriptor／Relation予定値を検証し、hash／length確定、close、同じRun root内の確定staging pathへの非上書きrenameがすべて成功した後だけ、opaqueな`CaptureRunArtifactStagingFinalizationReceipt`を厳密に1件発行する。Receiptは発行writer instance identity、TestRunId、Run Token、Descriptor全値、Frame Relation全値、確定relative pathへ参照同一性でimmutableに結合し、別writer、別Run、別Token、別Descriptor／Relation／pathへの置換、再発行、複製、二重消費を拒否する。`CaptureRunArtifactCompletion`はこの正確なReceiptなしに構築・登録できない。NVENC確定後のstaging／final verificationとpublishは共通`ICaptureArtifactStore`へ戻すが、`WriteStaging`やそのdurability receiptを偽装しない。Phase 0.11 Receiptはprocess-levelのclose／rename成功だけを証明し、`Flush(true)`またはOS crash／電源断durabilityを証明しない。

Draft、Registry、Trace の状態遷移は main thread の共通 Coordinator だけが反映する。Backend はこれらを変更しない。Draft の外部状態は `Pending / Staged / Dropped` を維持し、backend 内部状態を追加しない。形式非依存 Drop Reason は既存 enum へ append-only で追加し、既存 0～9 を再番号しない。Freeze は新規受付停止、Backend `BeginDrain`、Frame／submission Artifact Completionの完全回収、queued cancellation、`TryJoin`、`CaptureRunArtifactCompletion`登録またはAbandoned予約解放、main-thread terminal intent反映、残存Pendingのtimeout Drop、Trace seal、Publicationの順とする。Join 前に backend-owned Surface／buffer／未確定chunk payloadを外部からDisposeしない。

Phase 0の境界確定では将来backendを差し替えられる責務分離だけを確定する。ハードウェアencoder、特定動画codec、profile／bitrate／rate control、packet／GOP／keyframe、PTS／DTS／reorder、container／segment、GPU native handle／zero-copy、独自binary schema、MessagePack／CBOR／Protobuf、worker thread／Job／Burst、queue実測調整はPhase 0の設計・実装・比較・Spikeに含めない。Phase 0.1は既存境界を維持する。Phase 0.11はPhase 0完了後の破壊的変更として`ICaptureRunArtifactProducer`と`FrameSequence` Kindだけをappend-only追加できるが、既存`ICaptureEvidenceSession`、Envelope、Frame／submission Artifact Completion、Publication Planの意味を変更しない。特定codecの生成／verificationとStore実装はBackend内部へ隔離する。
...
Phase 0.11はPhase 0.1で確定した非同期Capture境界を入口条件として使用し、Phase 0のPNG＋JSON Backendを変更しない。複数Frame共有Artifactを既に表せる`CaptureArtifactFrameRelation`と、21.15で追加した最小Run Artifact拡張だけを使い、Capture FrameとFrameId／Traceの対応、Freeze／Drain／Join／Publication／Recovery／CaptureCompleteをBackend固有経路で迂回しない。指定GPU／Driver／SDKを持つhardware-qualified runnerのnominalだけは`BringUpCadenceTickCount=120`、`BringUpCadenceHz=30`、`SubmissionWindowDeadlineMs=4000`、`FinalizationDeadlineMs=30000`を固定する。Tick indexは`0..119`、予定時刻はmonotonicな開始時刻を`T0`として`T0 + index / 30 second`とし、各tickで提出を厳密に1回だけ試みてBackpressure時も同tickを再試行しない。tick 119の試行完了または`T0 + 4000 ms`到達の早い方で新規提出を停止し、その後Drain／Join／chunk確定／Publicationへ進む。Finalization期限は提出停止時点から測り、期限内に期待終端へ到達しなければscenario失敗としてCaptureだけをAbortする。この4秒は提出窓のhard boundであり、Finalizationを含む全Run時間を4秒と主張しない。一般CIではmock／fake completion sourceを使う短い決定論的lifecycle試験だけを必須とし、wall-clock 30fps、NVENC capabilityまたは120 Frame成功を要求しない。同一Runで提出を再開しない。

hardware-qualified `NvencNominalBringUpV1`は全120 tickについて提出試行、Accepted Work Token、encode Frame Completionを厳密に1対1で要求し、Frame Completionは全件`Succeeded, ProducedArtifactCount=0`とする。正常Drain／Join後に、全120 FrameのAccess Unitをaccepted CaptureFrameId順に含む確定済みRun chunk Artifactを厳密に1件要求する。chunkのpath、length、hash、TestRunId、Run Artifact Tokenと、正のCaptureFrameId 120件を重複なくaccepted順に持つFrame Relationを検査し、1件の`CaptureRunArtifactCompletion`、Run専用Registry登録、Publication、CaptureCompleteが30秒Finalization期限内に成功しなければPhase 0.11のnominal完了を認めない。受付拒否、Backpressure、Drop、encode／chunk確定失敗、件数不足をnominalの許容劣化にしない。この試験は対応hardwareのPhase承認／定期qualificationで実行し、GPUを持たない一般CIのmerge Gateにしない。
...
`NvencBringUpProfileV1`は`NvencWorkSlotCount=8`と`NvencInputSurfaceSlotCount=4`を固定し、Run開始前にWork Slot領域とNVENC Input Surface Poolを生成する。各Input slotのD3D11 Texture／NVENC登録handle／所有状態と、各Work Slotのtoken generation／inline Completion Cell／所有権状態を固定長領域へ保持する。`ICaptureEvidenceSession.TrySubmit`が受理した時点で、呼出側の`CaptureSurfaceLease`所有権はBackendへ移る。BackendはSource Textureを読むcrop／flip／RGBA→NV12 GPU変換を登録しただけではLeaseを解放しない。GPU queue上で当該変換がSource Textureを参照し終えたことを示す`SourceReadCompleted` Fence／Query等の非同期完了証拠を取得した後だけ、Backend内部の軽量Completion処理がLeaseを元Surface Poolへ返却またはDisposeする。これはSource所有権だけの解放であり、この時点では`CaptureFrameCompletion`を発行しない。Main／Render Threadは証拠を待たず、未完了Leaseを元Surface Poolへ返さない。

`NvencCaptureEvidenceBackend`は`MaximumArtifactCountPerSubmission=0`と`MaximumRunArtifactCount=1`を宣言し、各固定Work SlotへFrame Completion Cell 1個だけをinline保持する。Run開始時はCoordinatorからatomic factoryが同時発行した`CaptureRunArtifactToken + CaptureRunArtifactReservationReceipt`だけを受け取り、その正確なTokenとProducer identityへ結合した固定容量`INvencChunkStreamingStagingWriter`を生成する。この3者を一括確保できなければ新規Frameを受け付けず、Coordinatorが同じToken／receiptで予約を解放する。Backendが独自にRun Tokenまたは予約を後付け発行しない。`TrySubmit`内部で空Work Slot、NVENC Input Slot、未使用Frame Cellその他必要資源を一括予約できた場合だけ`Accepted`を返し、途中で容量不足、`NotAccepting`または例外となった場合は取得済み資源を逆順で全返却し、Surface所有権を受け取らず無効tokenを返す。Source Surfaceの解放とNVENC Input Slotの再利用は別のlifecycleである。Source Read完了後もNV12を保持するInput Slotは占有を継続し、NVENC encode完了証拠を取得して圧縮bitstreamの所有権を専用chunk writerへ移した後だけ再利用する。encode成功、返却bufferが非null／非空かつ1 Access Unit上限以下であり、accepted CaptureFrameId順のappend ownershipをchunk writerへ移転できたproducerは、Frame Cellの`Reserved -> Writing`を取得して`CaptureFrameCompletion(Status=Succeeded, ProducedArtifactCount=0)`をexactly onceで公開する。encode失敗、取消、null／空出力、Access Unit上限超過、chunk byte上限超過またはappend ownership移転失敗ではFrame Cellへ`Failed`または`Cancelled, ProducedArtifactCount=0`をexactly onceで公開し、Run chunkを`Abandoned`へ固定して新規受付を停止する。Frame WorkはArtifact Descriptor、content hash、Artifact Completionを生成しない。取消、encode失敗、Drain時もSource Read完了前のLeaseとNVENC完了前のInput Slotを早期返却せず、device loss等で通常証拠を得られない場合は再利用可能Poolへ戻さず失敗終端してBackend破棄時に隔離解放する。

NVENC inline Frame Completion Cellの状態はWork Slot generationごとに`Empty -> Reserved -> Writing -> Ready -> Consumed`だけとする。成功／失敗／取消の競合は`Reserved -> Writing`の一方だけが勝ち、敗者はFrame payload、状態、所有資源を変更しない。producerはCompletion payloadと対応token generationを完全構築した後、release semantics相当で`Ready`を公開する。Main Thread collectorはacquire semantics相当で`Ready`を観測した後だけpayloadを読み、読取り前とlocal copy後の両方でpayload token generationとWork Slot generationの一致を検証してから、`Ready -> Consumed`をexactly onceのatomic遷移として確定する。不一致、部分初期化または既にConsumedのCellをCompletionとして返さない。Work Slot再利用はFrame Cellが`Consumed`、全Source／Input／Access Unit所有権が安全に解放またはchunk writerへ移転済みであることをacquire semantics相当で確認した後だけ許可し、その後にslot generationを進めてCellを次generationの`Empty`へ初期化する。`Volatile`、`Interlocked`、lock-free primitive、短時間lock等の実装手段は固定しないが、上記release／acquire、単一producer claim、exactly-once consumeと同等以上を必須とする。
...
crop、orientation補正、RGBA→NV12は可能な範囲で同一のversion付きGPU conversion passへまとめ、CPU roundtripを禁止する。API／format制約で追加GPU copyが必要な場合はPass数、copy数、Texture遷移を固定ProfileとProfilerへ記録するが、zero-copy方式比較や複数実装の最適化をPhase 0.11へ持ち込まない。構造テストでは`NvencCaptureEvidenceBackend`が`UnityRenderTextureReadbackDispatcher`を参照しないこと、Main／Render Thread経路から待機可能APIとArtifact Storeを呼ばないことを検査し、ProfilerでNative callback時間、Source Surface／Input Slot占有、Backpressure数を取得する。Phase 0.11の必須lifecycle回帰は、正常120 Frame chunk、Work／Input容量不足、Source Fence遅延、NVENC Completion遅延、encode失敗、chunk確定失敗、pending Workを含むDrainの7代表caseへ限定し、失敗位置、残容量、取消を直積化しない。容量不足caseではBackend `Backpressured`を容量回数以上反復してもWork Slot、Input Slot、Run Artifact予約が減らずcallerがSurfaceを保持することを確認する。`NotAccepting`と例外のrollback、foreign／stale Frame Token／Run Tokenは短い共通Backend lifecycle testで各1 caseだけ検査する。遅延caseではSource Read証拠前に元`CaptureSurfaceLease`が再利用されず、Source Readだけの完了ではLeaseだけが解放されInput SlotとFrame Cellが未完了であることを確認する。成功時は各Frameが`Succeeded/count=0`を厳密に1件返し、正常Drain／Join後にだけRun Artifactが`Finalized`となって有効なStaging Finalization Receipt付き`CaptureRunArtifactCompletion`を1件返す。失敗／取消Frameまたは0 Accepted Frameではchunkを`Abandoned`としてRun予約を1回だけ解放し、Run Completionを返さない。atomic factoryによるToken／予約同時発行、Backend初期化失敗後の解放、別Producer／Run／Tokenへの置換、再結合、Run予約成功後のFinalized登録、Abandoned解放、foreign receipt、二重登録、二重解放、Frame Token流用を検査する。さらにclose前、rename失敗、別writer／別path／改変Descriptor／Relation、偽造・再利用Staging Receiptをすべて登録前にRejectし、共通Storeの`WriteStaging`がNVENC経路から呼ばれないことを確認する。Drain後にWork Slot、Frame／Run Artifact予約、Lease／Input Slot／Access Unit buffer／writer handle／Staging Receiptの二重返却、欠落、永久占有がないことを確認する。inline Frame Cellの専用concurrency回帰ではpayload各fieldの書込み途中、release直前／直後、collectorのacquire直後、`Ready -> Consumed`競合、成功対失敗／取消の`Reserved -> Writing`競合、旧generation producer遅延を決定論的barrierで挿入する。一方だけが勝って敗者がFrame状態と資源を変更せず、default／部分payload、古いgeneration、二重回収、早期Slot再利用が一度も観測されないことを検査する。追加の組合せFault Injectionは不具合再現または実測で必要になった場合だけPhase 4.8へ追加する。

V1 cardinality回帰では`MaximumRunArtifactCount`の未設定、0、1、2、Run中の1から別値への変更を与え、1だけがToken／予約receipt／writerを各1件生成し、それ以外ではfactory呼出し前に全て0件のまま開始拒否となることを確認する。複数Run Artifactまたは2番目のchunkを要求するProfileはPhase 0.11でRejectし、Phase 4.8の別versionなしに暗黙受理しない。
...
正常Freezeでは新規受付停止後にAccepted WorkをDrainし、全Frame Completionを回収してNVENC producerをJoinする。Run chunkを`Finalized`にできる最小条件は、(1) AcceptedかつSucceededのFrameが1件以上、(2) Access Unit append件数が1件以上、(3) Succeeded Frame数、append済みAccess Unit数、Frame Relation件数が一致し、同じaccepted順であること、(4) 全Accepted FrameがSucceededであること、とする。0 Accepted Frame、0 Access Unit、件数／順序不一致またはいずれかのFrame失敗では空chunkを確定せず`Abandoned`へ進む。

最小条件を満たした場合だけ、専用Workerは`INvencChunkStreamingStagingWriter.TryFinalize`を呼ぶ。writerはappendを停止し、appendごとに更新済みのchecked累積ByteLengthと単一streaming content hashを確定してhandleをcloseし、確定staging pathへ非上書きrenameした後だけ、Descriptor／Relationへ結合した`CaptureRunArtifactStagingFinalizationReceipt`を返す。hashのためにfileを再読込せず、`Flush(true)`を呼ばず、close後にhandleを再openしない。BackendはこのReceiptと完全一致するDescriptor／Relationから`CaptureRunArtifactCompletion`を構築し、Run outcomeを`Finalized`へrelease semantics相当で公開する。Main Thread Coordinatorはacquire後にRun専用Registry予約とStaging Receiptをexactly once消費してCompletionをPlan用Descriptor／Relationへ登録し、その後のVerify／Publishだけを共通Storeへ委ねる。Frame Completionより`CaptureRunArtifactCompletion`が先に収集されることを禁止する。encode、append、hash、close、rename、Receipt／Descriptor／Completion生成またはJoinのいずれかが失敗・取消となった場合はchunkを`Abandoned`へexactly onceで固定し、`.partial`またはPlan未登録の確定staging fileをbest-effortで削除してRun専用予約をexactly once解放する。この経路ではCompletion、Descriptor、Plan Entryを生成せず、ゲーム、既に確定した別Artifact、Frame Completionをrollbackしない。Trace enqueue失敗はRun IncompleteとするがFrame／Run Artifact outcomeを再発行しない。Freeze Receiptは全Frame CellがConsumed、全Work／Input／bitstream所有権が解放済み、Run Artifact outcomeが収集済み、Frame／Run Artifact予約と未消費Staging Receiptが0である場合だけ発行する。
