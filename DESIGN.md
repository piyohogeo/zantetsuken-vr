...
| NvencEncodeSampleSlot | Phase 0.11の1件のin-flight encodeへ排他的に束縛するCapture専用固定資源。NV12 Input Surface、D3D11／NVENC登録・mapped所有状態、Output Bitstream Buffer、Completion Event、GPU変換完了証拠を論理的に対応付ける。Work Slotとは別Pool／別lifecycleで固定8組を持ち、reorder bufferには使わない |
