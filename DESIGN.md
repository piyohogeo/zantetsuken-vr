...
| Phase 0.11 | 最小NVENC bounded chunk Backend | D-137～D-139の`NvencBringUpProfileV1`とWindows／NVIDIA／D3D11固定の`NvencCaptureEvidenceBackend`をPhase 0.1の非同期境界へ追加する。指定hardware-qualified環境のnominalだけを30fps／120 tick／4秒提出窓／30秒Finalizationとし、一般CIのlifecycle試験とfault系は短い決定論的caseまたは最大16 tick／1秒提出窓／10秒Finalizationとする。GPU TextureからNV12変換してNVENCへ渡し、PNG用RGBA CPU readbackを経由しない。Main／Render ThreadでGPU／NVENC完了、bitstream、hash、I/Oを待たず、各Frameのraw Annex B Access Unitをaccepted順に単一`NvencCaptureRunCoordinator`所有の`NvencRunChunkContext`へappendする | hardware-qualified nominalは120件すべてのAccepted／Frame Completion、1件の確定chunk Artifact、120件のFrame Relation、Contextの`Finalized` terminal result、局所Registry slotの`Registered -> Committed`、Plan commit、Publication、CaptureCompleteを要求する。全Frame Completion回収後、専用Worker停止前にchunk単位でhash確定、close、rename、terminal result生成を各1回行い、その後`TryJoin`する。`Abandoned`または既知pre-commit失敗では通常Plan／CaptureCompleteへ進まず`Incomplete`、rename結果不明ではfileを変更せず旧process内の`CommitOutcomeUnknown`とし、Service静止後にLeaseを解放して新processのfile集合分類へ委ねる。H.264 High、IDR-only、SPS／PPS反復、P／B FrameとFrame間参照なし、CQP 28を要求設定として記録し、raw bytesを変更しない。`Flush(true)`は要求しない。0 Frameまたは未確定chunkは全体を破棄してよい。clean Decoder process 1回で確定chunkから120 Frameを順次decodeし、基本寸法／orientation／色を確認する。Render callbackはboundedな登録だけで戻り、固定Surface／Work／Completion／chunk容量枯渇時はBackpressureまたはCapture失敗終端となる。このbounded chunk形式を連続運用へ昇格しない |
...
Phase 0.11の責務は次の3系統へ固定する。ここで「単一Consumer Worker」はCapture pipeline内の単一論理Consumerを意味し、Consumer Join後にPlan I/Oを行う非所有Publication Service Workerとは別である。Main／Render Thread Producerは固定容量資源をAccepted前に予約して`TrySubmit`し、待たずに戻る。Capture Consumer WorkerはSubmission Queueを単独消費してGPU完了回収、Source Surface返却、NVENC完了回収、Input Surface返却、bitstream ownership取得、accepted順chunk append、streaming hash／ByteLength更新、Frame Completion生成を行う。Main Threadは固定容量Completion Queueをbounded pollしてDraft／Traceの正式状態だけを反映する。単一`NvencCaptureRunCoordinator`は両Queue、Backend／Consumer、Context、Session Ownership Lease、Trace Freeze状態、局所Registry slotおよびDispositionをRun開始からPlan commitまたはIncompleteまで所有し、Consumer静止後のPlan I/Oは所有権を渡さずPublication Serviceへ非同期要求する。

```text
Main／Render Thread Producer --TrySubmit--> 固定Submission Queue
                                              |
                                              v
                                   Capture Consumer Worker
                                              |
                                              v
                                   固定Completion Queue
                                              |
...
Phase 0.11の停止順は`StopAccepting -> BeginDrain -> Accepted済み処理とFrame Completionの完全回収 -> Consumer上でchunk FinalizeまたはAbandon -> Context terminal result回収 -> Consumer Join -> 全資源／予約ゼロ確認 -> Trace Freeze -> Plan commitまたはIncomplete`とする。Finalize／Abandonまたはterminal result回収をConsumer Join後へ送らず、Join成功後に同Consumerへ新しい処理を要求しない。
...
hardware-qualified `NvencNominalBringUpV1`は全120 tickについて提出試行、Accepted Work Token、encode Frame Completionを厳密に1対1で要求し、Frame Completionは全件`Succeeded, ProducedArtifactCount=0`とする。正常Drainで全Frame Completionを回収した後、Worker停止前に全120 FrameのAccess Unitをaccepted CaptureFrameId順に含むRun chunkを確定し、Context terminal回収と局所Registry登録後に`TryJoin`する。chunkのpath、length、hash、TestRunIdと、正のCaptureFrameId 120件を重複なくaccepted順に持つFrame Relationを検査し、Contextの単一`Finalized` Result、局所Registry slot=`Committed`、Disposition=`Committed`、Publication、CaptureCompleteが30秒Finalization期限内に成功しなければPhase 0.11のnominal完了を認めない。受付拒否、Backpressure、Drop、encode／chunk確定失敗、件数不足をnominalの許容劣化にしない。この試験は対応hardwareのPhase承認／定期qualificationで実行し、GPUを持たない一般CIのmerge Gateにしない。

fault、Backpressure、Freeze／Drainの各scenarioは`FaultCadenceTickCount=16`、`FaultCadenceHz=30`、`FaultSubmissionWindowDeadlineMs=1000`、`FaultFinalizationDeadlineMs=10000`を上限とする。必要tick数をscenario開始前に`1..16`で宣言し、index `0..DeclaredTickCount-1`を`T0 + index / 30 second`で各1回だけ試みる。宣言tick末尾または`T0 + 1000 ms`の早い方で提出を止め、提出停止後10秒以内に期待Dispositionへ到達させる。事前宣言したFault Injectionと期待Dispositionに一致するAccepted／Frame Completion件数不足またはRun chunk `Abandoned`だけを許し、期待値にない不足、期限後の追加提出、同tick再試行、fault解除後の窓延長は禁止する。10秒超過はscenario失敗と新規Capture処理停止を確定するだけであり、未静止Publication ServiceのSession Leaseを解放する期限ではない。
...
crop、orientation補正、RGBA→NV12は可能な範囲で同一のversion付きGPU conversion passへまとめ、CPU roundtripを禁止する。API／format制約で追加GPU copyが必要な場合はPass数、copy数、Texture遷移を固定ProfileとProfilerへ記録するが、zero-copy方式比較や複数実装の最適化をPhase 0.11へ持ち込まない。構造テストでは`NvencCaptureEvidenceBackend`が`UnityRenderTextureReadbackDispatcher`を参照しないこと、Main／Render Thread経路から待機可能APIとArtifact Storeを呼ばないことを検査し、ProfilerでNative callback時間、Source Surface／Input Slot占有、Backpressure数を取得する。Phase 0.11の必須回帰は次の10代表caseに限定する。(1)正常120 Accepted Frameが1 chunkとして確定、Trace Freeze、Plan commit、Publicationされる、(2)pre-finalize失敗でPlanを作らず`Incomplete`となる、(3)chunk Finalize後かつrename呼出し前の失敗で局所Registry slotと予約を解放し`Incomplete`となる、(4)NVENC専用tmpだけを残した再起動で自動昇格しない、(5)完成Planだけを残した再起動で通常Recoveryできる、(6)完成PlanとNVENC専用tmpの同時存在をcollisionとして無変更停止する、(7)commit後のPublication失敗でPlan／chunkを削除せず`PublicationRecoveryRequired`となる、(8)abort cleanupのfile削除が失敗してもゲームを止めずLeaseを解放する、(9)同一Runで局所Registry登録とPlan commitを重複実行しない、(10)Main／Render ThreadからPlan I/O、cleanup、待機可能API、hashまたはfile I/Oを呼ばない。別途、rename結果不明を`CommitOutcomeUnknown`へ送り、その場で再読込・cleanupせず次回Recoveryへ委ねるcaseと、確定chunkをclean Decoder process 1回でdecodeするsmoke caseを置く。容量不足caseは反復してもWork Slot、Input Slot、Completion credit、Context予約が減らずcallerがSurfaceを保持することを確認する。遅延caseはSource Read証拠前にLeaseを再利用せず、Source Readだけの完了ではLeaseだけを解放してInput SlotとFrame Completionを未完了とする。成功時は各Frameが`Succeeded/count=0`を厳密に1件返し、全Frame回収後かつTryJoin前だけContextが`Finalized` Resultを1件生成する。失敗／取消Frameまたは0 Accepted Frameでは`Abandoned`として予約を1回だけ解放し、Resultを生成しない。Frameが全てStagedでもContextが`Abandoned`または局所Registry未登録ならArtifact 0件のPlanを構築せずCaptureCompleteを発行しない。Session Ownership Leaseの基本試験は通常Session alias、非所有Lock Identity、foreign／stale Lease、二重DisposeからOS lockを解放できないことに限定し、Coordinator間のbundle移譲、owner generation変更、Registry handle再発行、`PlanConsumed`または送受信組合せ試験を置かない。初期化失敗、SPSC Queue wraparound、Frame Token重複、Contextの二重終端を短い単体試験で検査する。資源ゼロおよびWork Slot／Lease／Input Slot／Access Unit buffer／writer handleの意図しない永久占有なしは、全native／OS処理が帰還して制御可能なterminal処理へ到達したcaseだけで検査する。fake native completion gateを閉じた恒久停止caseでは、期限超過後も偽Frame Completion、credit／GPU資源／OS lockの再利用・解放、Trace Freeze、Plan commit、`Incomplete`終端または成功Joinが発生しないことを検査する。固定容量枯渇後は`Backpressured`または当該Runの受付停止となりMain／Render Threadとゲーム側を待たせない。gate解放後にnative処理を帰還させた場合だけterminal Completionをexactly onceで発行し、制御可能なDrain／Joinへ収束することを確認する。複数Run Artifactまたは2番目のchunkはPhase 0.11の構造上生成できず、Phase 4.8の別versionへ送る。

Plan境界の回帰では、Phase 0.11のpre-commit失敗後に`publication.plan.nvenc-precommit.tmp`削除も失敗させて再起動し、canonical bytes、正しいRun相関および対応chunkが存在しても`publication.plan`へ昇格せず隔離／破棄されることを確認する。同じsuiteでPhase 0／0.1のcanonical `publication.plan.tmp`だけが残るcaseは従来どおり昇格できることを確認する。Phase 0.11専用renameが例外または結果不明を返すcaseは旧process内で`CommitOutcomeUnknown`へ固定し、final／tmpの再読込、Registry slot破棄、staging削除または再renameを行わない。Service request実行中、rename call未帰還またはterminal result未回収ではSession Leaseを解放できず、terminal result後はService-owned handle、request slot、bufferおよびI/O commandがすべてゼロであることを検査する。timeoutと明示取消でもWorker静止前に同processまたは別processのRecoveryがroot lockを取得できないことを検査する。terminal resultとLease解放後に再起動し、完成Planのみ／専用tmpのみ／Planなし／final＋tmp／finalとchunk不一致をそれぞれ`PublicationRecoveryRequired`／Incomplete-orphan cleanup／Incomplete-orphan cleanup／`PublicationRecoveryCollision`／Recovery collisionへ分類する。directory metadata flushをPhase 0.11成功条件にせずPhase 0／0.1では引き続き必須とする。完成`publication.plan`と`publication.plan.nvenc-precommit.tmp`を同時に配置したcaseでは、final、tmp、chunkその他Run root内の全fileの存在、長さ、content hashが検査前後で不変であり、Plan／Artifact公開とCaptureCompleteが発生しないことを確認する。
