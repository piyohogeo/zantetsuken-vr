...
`TemporaryRenderCapRecordSet`、Cap Bounds Polygon、StencilのColor割当て、Cap板側のDraw Listは、描画更新境界ごとに`SelectedTemporaryClipPlaneSet`に選ばれた境界だけから構築する。Stencil Volumeは既存の可視性・Color分類に従い、選択済み境界でclipしたRenderFragmentごとにsubmitする。候補追加、LogicalCutOperation公開、Geometry Commit等で選択結果が変わった場合は、同じ描画更新境界で全体を再構築し、既存配列からの個別除去、有効フラグ、Buffer compaction、部分更新を行わない。Ignored境界の即時断面は、背景Geometry Commitまたは選択への編入まで表示されない。通常Colorでは別RecordのResidual Stencilが到達し得る非互換Capを分離し、最後の統合Colorでは混入、欠落、余計なCapおよび誤ったDepthを許容する。Ignored境界の断面を補うための追加Mesh生成、代替VFXは追加しない。
...
8. `MaxStencilColors`の最後の統合Colorでは、混入、欠落、重複、誤ったDepth／Occlusionを許容する。
...
- Broadphaseでは分離Offsetと安全Marginを含む物体OBBの左右眼投影矩形を使う。重なる組だけ、表向きのOBB切断面から得たCap Bounds Polygonを左右眼へ投影して再判定する。Frustum／Facing Cullで省略したCapを持つ対象は、Cap Bounds Polygonによる再判定を使わずOBB投影だけで判断する。選択済み境界だけから描画用Capを構築することは、この可視性による省略を補わない。どちらの判定も非交差なら安全という悲観的な証明として扱い、Near Plane交差、Raster／MSAA、頭部移動誤差を考慮してBoundsを保守的に拡張する。
...
- Cap処理は`受付済み未Commit面・SideからRecord構築 -> TemporaryClipConstraintCandidateSetのstable選択 -> 両眼Frustum／Facing Cull -> CapCompatibility Group -> 全Cap不可視Group Cull -> 通常Color割当て／最後のColor統合 -> Colorごとの128初期化／選択対象の全Volume／全Cap描画`の順とする。Operation公開時に同じ採用面のRecordへ一度だけ引き継ぐ。同じColor内のVolume→Cap順を維持する。Plane容量超過で論理状態を変更せず、描画用Cap Record集合は選択結果から描画更新境界で再構築する。
