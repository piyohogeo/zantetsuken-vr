...
骨Physics Proxyの現在pose component検証は`docs/diagnostics/compact16uv-character-physics/`。同じ固定Casual／Professionalの38 UCXをRenderer-bind-local→Unity bindposeによるbone-local→現在Renderer-localへ写し、実import済みUCX階層のWorld頂点と8条件で照合した。最大World差1.125e-6、合格35形状で計280回のJob切断／再切断・cook呼出しが成立し、EditMode全件3,630件が合格した。ただし元形状のCasual左右大腿とProfessional左上腕は既存凸性監査に不合格であり、全キャラクターの物理登録は保留する。不合格3形状を省いて完成したownerとはしない。各形状の隣接2面の対角線を交換する候補は、元頂点位置と監査許容を保持した計算上で3件とも凸性を回復したが、元アセット／fixtureへ未適用である。アセット側の修正と新SHAによる再監査を先行し、製品に暗黙の再convex化や許容緩和の救済経路を入れない。

上記は元入力に対する初回検証の履歴である。2026-09-24のアセット側commit `5233b74`で3対角線だけを修正した別出力を作成し、製品worktreeの`Assets/Licensed/Compact16uvConvexRepair/`へ新SHA・新GUIDで固定した（`docs/diagnostics/compact16uv-character-physics-repaired/`）。元intake／fixtureは保持する。修正版は全38形状が元座標・Renderer-bind-localの凸性監査を通り、Unityの8 pose条件で全形状の写像、cook呼出し、計304回の単体Job切断／再切断が成立した。Unity import後の表示頂点・normal・tangent・UV・index・weight・bindpose・bone順序は元版と完全一致し、UCX三角形も向き込みで修正fixtureと一致した。元版8条件と修正版8条件を併走し、元版の3拒否を消して合格化していない。形状由来の拒否は修正版で解消したが、19形状をまとめたowner登録、表示と同じ面による切断・所属・commit、scene／Rigidbody検証は未完了であり、製品参照先はまだ切り替えない。IL2CPP／性能／従来のCasual1画素差は本検証の対象外とする。

続く19形状compoundの共通面・実commit検証は`docs/diagnostics/compact16uv-character-compound/`。Casualの基準／骨回転poseは既存Driver、Registry、Cook、DAG、表示commitを通り、各2回の切断・再切断、正負へのSplit／継承、旧owner退役、child公開、体積・質量和、表示追従が成立した。3つのメモリ初期状態で計6ケース・12commitが一致した。一方、Casualの祖先scale付き2条件とProfessionalの全4条件は、表示の`lineageToGeometryLocal`がscaleを含み、既存の剛体限定入力契約で登録時に拒否された（F-CHARACTER-LINEAGE-SCALE）。18件の期待拒否テストは安全な停止の確認であり、統合合格ではない。表示頂点のRenderer-local保持と物理Ownerの剛体frameをつなぐ契約の検討が次工程である。表示側のlineage写像をscale対応へ拡張するか、共通剛体frameへのGeometry変換を追加するかは未決定であり、今回Runtime契約は変更していない。実simulation、Player表示／性能、製品sceneの参照先切替は引き続き未完了。

その後の固定scale正規化プローブは`docs/diagnostics/compact16uv-fixed-scale/`。Professionalの固定0.01は、読み込み準備時に頂点を`s`倍、bindposeを右から`Scale(1/s)`倍し、元の骨・UCX階層を共有した別Rendererをunit-scale親の下へ置く方式で解消した。骨階層全体のscaleを1にするのではなく、切断入力／表示GeometryのRenderer-local単位を正規化する。元アセット・fixtureは不変。Casual / Professional各300決定的姿勢で元版と準備版のUnity `BakeMesh(true)`を比較し、最大World位置差1.05e-6未満、法線ベクトル差6.47e-7未満。基準／骨回転／rootBone非一様scaleの計6条件×3初期状態で19形状compoundの実切断・再切断36commitが成立した。固定一様scaleは入力準備で吸収する方針を優先し、lineageの剛体契約は維持する。準備後の外側親へ動的な非一様scaleを加える12件は引き続き期待拒否であり、対応済みとは扱わない。Focused 32/32、全EditMode 3694/3694、skip 0。テスト専用のメモリ上変換で、製品Loader／Importer組込み、共有Meshキャッシュ、実Clip、Player表示・性能、自前Burst skinningの確認は次工程。切断ごとのscale補正／Main再packは追加していない。

Runtime入力境界への移行は`docs/diagnostics/compact16uv-fixed-scale-input/`。`VpFixedScaleSkinCache`は同じsource Mesh／固定scaleの準備Meshを共有し、scale 1なら元Meshを借用する。`VpFixedScaleSkinInput`はinstanceごとの骨とRenderer切替・復帰を所有し、全input解放後にcacheを解放する。同期Bakeは準備Rendererの`BakeMesh(true)`で、切断ごとのscale補正／Main再packは追加しない。専用Playerと既存統合テストへ接続し、36commit、Focused 40/40、全EditMode 3704/3704が成立。IL2CPP D3D11の3独立processでは各28/28画像比較が合格し、56画像が完全再現、元Skinned→準備Skinnedはpixel一致。ただし取り込まれた`CanonicalSource`は両familyとも全3580 curveが定数で、120時点のWorld頂点変化0。これは静止Clipの確認であり、実モーション／Animator駆動の検証完了とはしない。API単体の準備cache miss中央値はCasual 180.8／Professional 418.7 µs、元Mesh保持に伴う追加Mesh量は0／749,112 B（同一asset・scaleごと）。切断Main全体・全scene peakの測定ではない。通常Gameplay sceneへの寿命接続と物理ownerを含むPlayer、動くClip、Importerでの永続化、自前Burstは残工程。
