...
未接続・未確認（この単位では扱わない）。Final Physicsの公開と置換（実切断のCut／cookと正負Owner候補の未公開構築までは7.2の「実装状況」の範囲で成立しており、この表示経路へは未接続である）、動く子の配置をPhysicsから表示へ渡す接続、Characterのskinningと骨に付いたConvexとの接続、製品全体の構成根への接続、この経路でのPlayerと性能、統合後の追加検証。これらが残ることは、過去に別の条件で確認済みの描画条件を未確認へ戻す意味ではなく、過去のXR証拠を今回のCommit経路の証拠に流用する根拠にもならない。Phase 3全体および関連する受入れ項目の完了は意味しない。
...
- 実装状況（2026-09-20追記。D-181・D-183の当時の本文と成立範囲は変更しない）。
  ...
  - 未解決：O-034（製品の`MaxStencilColors`など。試験値8は製品値ではない）。実Geometry Commitとの接続は4.5.6の「実装状況」の範囲で成立しており、そこに残る未接続・未確認（Final Physicsの公開と置換。Cut／cookと正負Owner候補の未公開構築までは7.2の「実装状況」による。動く子の配置の受渡し、Character、構成根、Player・性能）が後続となる。
...
実装状況（2026-09-20追記。上の目標仕様と既存の決定本文は変更しない）。物理切断の数値処理・cookと、その成果物から作る正負Owner候補の構築までを2単位で実装し、いずれもlocal mainへ統合した。いずれも未公開までであり、公開処理（Final Physics Commitと論理公開の一体切替）は含まない。詳細はここにまとめ、関係する節からは本項を参照する。
...
未接続・未確認（この2単位では扱わない）。Provisionalと必要なD6、Provisional→Final handoff。Final PhysicsとLogical Publicationの一体切替、Sourceの現在対象終了・退役と8章のStale回収への接続。公開時の有効化と設定の適用、Cut DAG（4.5.6）との接続、表示の追従。Characterのskinningと骨に付いたConvex、製品全体の構成根、この経路でのPlayerと性能、統合後の追加検証。これらが残ることは、過去に別の条件で確認済みの項目を未確認へ戻す意味ではなく、今回の証拠をそれらへ流用する根拠にもならない。Phase 4全体および関連する受入れ項目の完了は意味しない。
...
Phase 3のうち、非同期切断Work（773bb09）、Cut DAGの骨組み（c97e43c）、実Geometry Commitと表示差替え（0df0392）の3単位は、2026-09-20に区切った（4.5.6の「実装状況」）。確認は静的Geometryと合成Final Physicsによるもので、Final Physicsの公開と置換（Cut／cookと正負Owner候補の未公開構築までは7.2の「実装状況」による）、動く子の配置の受渡し、Character、製品の構成根、この経路でのPlayer・性能は未接続または未確認である。Phase 3全体とT-006・T-083・T-074のPhase 3部分は完了扱いにしない。

Phase 4のうち、Cut／Cookの未公開成果物まで（74bbd8e）と正負Final Owner候補の未公開構築まで（2135bf4）の2単位は、2026-09-20に区切った（7.2の「実装状況」）。確認は合成入力によるもので、Provisional・D6・Final handoff、Final PhysicsとLogical Publicationの一体切替、公開時の適用、Cut DAG接続、表示追従、Character、製品の構成根、Player・性能は未接続または未確認である。Phase 4全体と関連する受入れ項目は完了扱いにしない。
