...
| Phase 0.1 | Capture非同期化 | Phase 0で完成したPNG＋JSON形式と`PngJsonCaptureEvidenceBackend`を維持し、固定Unity版でthread-safeと規定された現行`ImageConversion.EncodeNativeArrayToPNG`をWorkerから使う単一路線とする。PNG encode、canonical JSON、hash、durable stagingを固定容量Workerへ移し、Main Thread PNG Fallback、実行時Capability分岐、別PNG libraryを実装しない。Main Thread上の`TryCollect*`は固定容量Completion経路の軽量pollと正式状態遷移への反映だけを行い、final publication、Recovery、CaptureComplete、cleanupは既存Coordinatorの責務を維持する | 固定FixtureでWorker出力がlosslessに同じRGBA pixel、寸法、orientationを復元し、canonical JSON、FrameId／Trace相関と既存Loader／Verifier互換性を維持することを確認する。同じEncoderでもPNG圧縮bytes／hashのMain Thread時代との一致は要求しない。Worker encode失敗時はMain Threadで再試行せず該当Captureを失敗終端し、ゲームを継続する。所有権、Completion順序、Drain／Join、Freeze、final Publication／Recovery契約を維持し、過負荷時はCaptureだけをBackpressureまたはDropする |
| Phase 0.11 | 最小NVENC bounded chunk Backend | D-137～D-139の`NvencBringUpProfileV1`とWindows／NVIDIA／D3D11固定の`NvencCaptureEvidenceBackend`をPhase 0.1の非同期境界へ追加する。指定hardware-qualified環境のnominalだけを30fps／120 tick／4秒提出窓／30秒Finalizationとし、一般CIのlifecycle試験とfault系は短い決定論的caseまたは最大16 tick／1秒提出窓／10秒Finalizationとする。GPU TextureからNV12変換してNVENCへ渡し、PNG用RGBA CPU readbackを経由しない。Main／Render ThreadでGPU／NVENC完了、bitstream、hash、I/Oを待たず、各Frameのraw Annex B Access Unitをaccepted順にCoordinator所有の単一`NvencRunChunkContext`へappendする | hardware-qualified nominalは120件すべてのAccepted／Frame Completion、1件の確定chunk Artifact、120件のFrame Relation、Contextの`Finalized` terminal result、Registry直接登録、`FinalizedAndRegistered` disposition、Publication、CaptureCompleteを要求する。全Frame Completion回収後、専用Worker停止前にchunk単位でhash確定、close、rename、terminal result生成を各1回行い、その後`TryJoin`する。`Abandoned`またはRegistry登録失敗ではTraceをFrozenにできるが通常Plan／CaptureCompleteへ進まず`Incomplete`で終端する。H.264 High、IDR-only、SPS／PPS反復、P／B FrameとFrame間参照なし、CQP 28を要求設定として記録し、raw bytesを変更しない。`Flush(true)`は要求しない。0 Frameまたは未確定chunkは全体を破棄してよい。clean Decoder process 1回で確定chunkから120 Frameを順次decodeし、基本寸法／orientation／色を確認する。Render callbackはboundedな登録だけで戻り、固定Surface／Work／Completion／chunk容量枯渇時はBackpressureまたはCapture失敗終端となる。このbounded chunk形式を連続運用へ昇格しない |
...
Contextの終端結果はauthority Receiptではなく、Contextだけが内部状態から構築できるimmutableな`NvencChunkFinalizationResult`とする。Resultは有効な`CaptureArtifactDescriptor`、accepted順の`CaptureArtifactFrameRelation`、確定staging relative pathを保持し、呼出側からDescriptor、Relation、path、ByteLengthまたはcontent hashを注入できない。状態遷移は`Open -> Finalized`または`Open -> Abandoned`だけとし、両終端は相互排他的かつexactly once、終端後append、再Finalize、Finalized後AbandonまたはAbandoned後Finalizeを禁止する。terminal Resultの全field構築後に短いlockまたはrelease semantics相当で終端状態を公開し、Coordinatorは同じlockまたはacquire semantics相当のbounded poll後だけResultを読む。Coordinatorは自身が所有するContextの`Finalized` ResultをPhase 0.11専用の内部`TryRegisterFinalizedRunChunk(context, result)`でRegistry予約へexactly once登録し、成功時にprocess-localな`NvencRunChunkRegistryEntryHandle`を受け取る。このhandleは正確なRegistry instance、TestRunId、Context、Finalization Result、Entry identity、元の予約、owner identity／generationへimmutableに結合され、`PlanConsumed`終端前だけ現在ownerが`TryCancelRegisteredRunChunk(context, result, handle)`で同じEntryをexactly once取り消せる。取消はEntry除去と元予約の解決を単一のRegistry lock下で行い、別Context／Result／Entry、foreign／stale owner、二重取消、`PlanConsumed`済みEntryを副作用前に拒否する。`Abandoned`では同じContextの未登録予約をexactly once解放し、Descriptor、Plan EntryまたはRun Artifact Completionを生成しない。Registry内部はFrame-scoped予約とは別にこの1件だけを保持し、Frame Tokenを偽装・流用しない。非公開constructor、単一所有、状態検査によって誤用を構造上防ぎ、汎用Token／Receipt相互認証やpartial-recovery状態を追加しない。

Phase 0.11専用Coordinatorはprocess-localな`NvencRunEvidenceDisposition`をappend-only固定値`None=0 / FinalizedAndRegistered=1 / Incomplete=2`で保持する。初期値は必ず`None`とし、未知値をfail-closedに扱ってTrace seal、所有権移譲、Plan構築またはPublication用Receipt発行へ進めない。成功候補ではContext terminal回収、Registry直接登録、`TryJoin`、全所有資源／予約ゼロ確認、Trace sealおよび後述のPublication Coordinatorへの所有権移譲を終えてもPlan commitまでは`None`を維持する。`None`のままPlan構築を開始できる唯一のauthorityは、正確な移譲済み`NvencFrozenRunPublicationOwnershipBundle`とする。Plan commit成功時にだけ`None -> FinalizedAndRegistered`、Plan commit前の明示Abort、Contextの`Abandoned`、Registry登録／Join／資源解放／所有権移譲の失敗、期限超過、Result／Relation不整合またはPlan pre-commit失敗では必要なRegistry取消後に`None -> Incomplete`を厳密に1回確定し、再代入しない。`Incomplete`ではPublication用Receiptを発行せず、通常Plan永続化、Capture Index確定またはCaptureCompleteへ進まない。Trace seal成功後に生じた所有権移譲／Plan pre-commit失敗を含む`Incomplete`理由は、生存processのRun結果またはRun外診断に限定し、Frozen Traceへの追記、新journal／receiptによるcrash-durable化を要求しない。rename済みでもPlan未登録のfileはorphanとしてbest-effortで破棄でき、Contextの`Finalized`状態を`Abandoned`へ書き換えない。
...
永続成果物の正本は `CaptureArtifactDescriptor` とする。Descriptor は ArtifactId、append-only の ArtifactKind、FormatId/Version、staging/final relative path、ByteLength、ContentHash を持つ。Artifact completion の Work Token は生成処理の provenance であり、Frame と Artifact の意味上の関係とは分離する。関係は独立した `CaptureArtifactFrameRelation` で表し、空集合を Run scoped、複数 ID を複数 Frame 共有 Artifact とする。`CapturePublicationPlan` は Run 相関、Descriptor 集合、この多対多関係だけを canonical に保持し、`.png`/`.json` 固定 field や「1 frame = 1 encoded file」を要求しない。Phase 0 では FrameImage と FrameMetadata の 2 Artifact を各 Frame に関連付けるが、別 backend は 0/1/複数 Artifact、複数 Frame segment、Run scoped Artifact を生成できる。例外として`NvencBringUpProfileV1`のPlan Builder入口は、Disposition=`None`、有効かつ未消費の`NvencFrozenRunPublicationOwnershipBundle`、Registry内の`ArtifactKind=FrameSequence`厳密1件、そのRelationが全Staged Frame ID集合と厳密一致することを必須とする。Artifact 0件、部分Relation、余分なFrame、別Kind、bundleなし、foreign／stale bundleまたは`None`以外ではPlanを構築しない。Plan commit後のPublication／Recovery入口は`FinalizedAndRegistered`と`PlanConsumed`済みRegistry handleを必須とする。

共通 Publication の Run lifecycle 正本は `CaptureEvidenceRunPublicationCoordinator` とする。Phase 0／0.1の既存 `CaptureFrameFreezeTerminalCoordinator.TryCompleteEvidenceRun` は従来どおり受付停止、queued cancellation、全Frame／submission Artifact Completion反映、backend join、未処理slotなし、Artifact予約ゼロ、Trace `Frozen`、同一Runの `CaptureRunInitializationSession` に対応する有効なOwnership LeaseによるOS lock保持を確認した後だけ `CaptureEvidenceRunFreezeReceipt` を発行する。Phase 0.11専用Coordinatorは受付停止、queued cancellation、全Frame Completion反映、Context Finalize／Abandon、terminal回収、`Finalized`直接登録または`Abandoned`予約解放、backend join、未処理slot／所有権／予約ゼロ確認、Trace Freeze、線形所有権移譲、Plan commitとDisposition確定の順で処理する。Phase 0完了後の破壊的な内部変更として、`CaptureRunInitializationSession`は`IDisposable`を実装せず、通常のSession参照、Freeze Receipt、Publication ResultまたはOpen Outcomeから直接OS lockを解放するAPIを持たない。実際の`CaptureRunLockLease`はowner identity／generationへ結合した非公開`CaptureRunInitializationSessionOwnershipLease`だけが内包し、その`Dispose()`だけがlockを解放できる。Receipt／Resultが公開するLock情報は参照同一性確認用の非所有`CaptureRunLockIdentityEvidence`へ置換し、実Lease参照やDispose能力を返さない。Session factory／Bootstrap／RecoveryはSessionと初期Ownership Leaseを同時に返す。Phase 0／0.1の既存Coordinatorと関連Result／ReceiptもSessionとLock Identityを非所有の相関証拠として保持し、別fieldの現在Ownership Leaseだけを解放・移譲するよう移行する。旧Lease、foreign owner、stale generation、二重解放を副作用前に拒否し、全既存Session生成・解放テストをこのLease契約へ更新する。

Phase 0.11専用の`TryTransferFrozenRunOwnership`は、正確なContext、Session Ownership Lease、同Leaseに対応する非所有Lock Identity、未消費の`CaptureEvidenceRunFreezeReceipt`、有効な`NvencRunChunkRegistryEntryHandle`およびDisposition=`None`を同一Run／RootLayout／generationとして検証し、既存Publication Coordinatorへall-or-noneで移す内部線形操作とする。成功時だけ元Freeze Receiptを一度だけ消費し、送信側のContext／Session Ownership Lease／Registry handle参照を原子的に空へする。Session Ownership LeaseとRegistry handleは同じunderlying Session／lock／Entryへ結合した次generationの受信側owner instanceへ置換し、旧instanceおよびそのaliasを失効させる。したがってRegistry Entry自体は終端消費されず、Plan commit前は受信側の新handleで取消できる一方、送信側や旧aliasからのDispose／取消／Receipt再利用は副作用前に拒否される。受信側はこれらを`NvencFrozenRunPublicationOwnershipBundle`として単一所有する。Dispositionは`None`を維持し、Publication用Receiptをまだ発行しない。失敗時は元Receiptを消費せずgenerationも進めず、両Coordinatorの所有権とDispositionを一切変更しないため、送信側がRegistry取消とabort cleanupを実行できる。

受信側のPhase 0.11専用`TryPersistTransferredFrozenRun(bundle, runManifestContentHash)`だけがDisposition=`None`でPlan構築を開始できる。Phase 0.11は共通`BuildAndPersist`を変更・流用せず、完全なcanonical Planを専用`publication.plan.nvenc-precommit.tmp`へ書く。Phase 0.11の論理Plan commit線形化点は、この専用tmpを同じRun rootの`publication.plan`へ非上書きrenameした瞬間とし、fileまたはdirectory metadata flushを要求しない。専用Store APIは`PreCommitFailed`と`Committed`を区別し、rename成功後の検証／receipt構築失敗を`PreCommitFailed`として返さず、lock下のbounded再読込で同じPlanを確認して`Committed`結果へ収束する。`PreCommitFailed`では受信側がbundle内handleでexact Entry取消、予約解決、file cleanup、Session Lease解放を行いDispositionを`Incomplete`へ確定する。`Committed`ではRegistry handleを`PlanConsumed`へexactly once終端化した後にDispositionを`FinalizedAndRegistered`へ確定し、初めてPublication用Receiptを発行する。commit後のpublish／verification失敗ではEntry取消やstaging file削除を行わず、既存Recovery／再試行へ所有権とSession Leaseを維持する。OS crash／電源断でrename済み`publication.plan`またはchunkを失うことはPhase 0.11の許容されたCapture喪失であり、durable commitを主張しない。Phase 0／0.1の共通`BuildAndPersist`は従来どおり`publication.plan.tmp`のdata／file metadata flush、非上書きrename、directory metadata flush完了までをdurable commit条件とする。

`IsFullyDrained`もqueued cancellation、Context terminal回収、Run chunk予約と未確定chunk所有権を含めて再判定する。Plan永続化は正確な移譲済みbundleまたはcommit後の有効なPublication Receiptを必須とし、Receiptと同じ `CaptureRunRootLayout` を保持する単一 `CaptureArtifactFileStore` をPlan Store兼Artifact Storeとして選択するため、Run AのPlanとRun BのArtifact Storeを組み合わせられない。再起動時は既存 `CaptureRunInitializationOpenOutcome` が `PublicationRecoveryRequired` とOS lock保持を証明し、同一RootLayoutである場合だけ同じStoreの `ReadOrRecoverPlan` から汎用 `CapturePublicationPlan` を復元する。CoordinatorはSnapshotのPlan TestRunIdがOutcomeとStoreのTestRunIdの両方に一致することを確認して `CaptureEvidenceRunRecoveryInspectionReceipt` を発行し、Recovery継続は同じCoordinator発行の有効なReceiptだけを受理するため、別Run Snapshotを差し替えられない。Phase 0／0.1の`publication.plan.tmp`だけが残る場合は従来どおり、固定上限内のcanonical文書でRun相関も一致するときだけ非上書きで`publication.plan`へ昇格できる。Phase 0.11の`publication.plan.nvenc-precommit.tmp`はcanonical、Run相関一致またはchunk存在の有無にかかわらず絶対に昇格せず、OS lock下で隔離または全体破棄する。両tmpの同時存在、tmpとfinalの不正な組合せ、別Run、上限超過はcollisionとして自動昇格せず停止する。共通tmp不正は変更せず報告し、lock下の明示的な`DiscardInvalidTemporaryPlan`だけが再検証後に正確なtmpを破棄できる。NVENC専用tmpはPhase 0.11のorphan／incomplete cleanupだけが正確なpathとno-follow検証後に破棄できる。読み取りは固定byte上限+1で停止し、JSON object化前の無割当preflightでArtifact数、Frame Evidence数、1 Frame当たりArtifact参照数、各文字列長を検査し、canonical再serializeと完全一致しない文書を拒否する。旧`PngJsonCapturePublicationPlan`系は既存Phase 0ファイルとの互換専用であり、新規共通Publication／Recoveryの正本にはしない。

`ICaptureArtifactStore` は形式を解釈せず、既存のreceipt付きstaging write、長さ/hash 検証、非上書き publish、staging/final verification を担当する。Phase 0／0.1の`WriteStaging`とそのflush／receipt契約は変更しない。形式固有妥当性は backend または形式別 verifier の責務とする。Recovery はPlanのDescriptor集合だけを走査して final 一致、確定済みstaging一致、欠落、不一致を分類し、不一致をcollisionとして停止し、正しい確定済みstagingがある欠落finalだけをpublishする。Backend固有の未確定`.partial`はDescriptorまたはPlanの一部ではなく、各Backendの明示契約に従って無視または破棄できる。CaptureComplete は全 Descriptor の final verification 合格後に限る。OS 固有 no-follow と directory metadata durability は Store backend の capability であり、path 文字列だけで保証済みと扱わない。

Phase 0.11だけは既存`ICaptureArtifactStore.WriteStaging`を呼ばず、Context内部の単一streaming writerを使用する。writerは同じ`CaptureRunInitializationSession`、OS lock、`CaptureRunRootLayout`およびpath安全規則へconstructor時に結合され、許可された単一`.partial` pathだけを非共有handleでopenする。append中はchecked ByteLength、streaming content hash、accepted CaptureFrameId列をwriter自身が更新する。専用Worker上の`TryFinalize`はappend停止、最小確定条件、Descriptor／Relation予定値を検証し、hash／length確定、close、同じRun root内の確定staging pathへの非上書きrenameがすべて成功した後だけContext内部の`NvencChunkFinalizationResult`を厳密に1件構築して`Finalized`を公開する。別writer、別Run、別Descriptor／Relation／pathを注入するAPI、Finalization Receipt、Run Artifact Completionを設けない。NVENC確定後のstaging／final verificationとpublishは共通`ICaptureArtifactStore`へ戻すが、`WriteStaging`やそのdurability receiptを偽装しない。Resultはprocess-levelのclose／rename成功だけを表し、`Flush(true)`またはOS crash／電源断durabilityを証明しない。

`Incomplete`のabort cleanupは、その時点で有効な`CaptureRunInitializationSessionOwnershipLease`とRegistry handleを持つCoordinatorだけが行う。所有権移譲前はPhase 0.11専用Coordinator、移譲後かつPlan commit前はPublication Coordinatorが責務を持つ。ContextがRegistry登録済みなら、OS lockを保持したまま正確な`NvencRunChunkRegistryEntryHandle`でEntry取消と予約解決を最初に完了し、取消不能または相関不一致では別Entryやfileへ触れずfail closedとする。未登録予約だけが残る場合は同予約をexactly once解放する。その後にContextの`.partial`、Plan未登録の確定staging file、`publication.plan.nvenc-precommit.tmp`、未消費buffer／handleをbest-effortで回収し、Context／writer／BackendをDisposeした後、`finally`相当で現在ownerのSession Ownership Leaseを厳密に1回Disposeして2本のOS lockを既存の逆順契約で解放する。通常のSession参照から解放を試みるAPIを設けない。Registry取消またはcleanup失敗でもlock解放を抑止せず、失敗理由はRun外診断へ記録し、Publication Receipt、Plan、Capture IndexまたはCaptureCompleteを生成しない。削除できなかった`.partial`、NVENC専用tmp、orphan、`run.init`／`run.ready`またはRun rootは残存を許容し、次回RecoveryではPlanのないrootを自動公開せず既存のorphan／incomplete相当として隔離または全体破棄する。Trace sealが再試行可能な間はDispositionを`None`に保ちSession Leaseとlockを保持し、seal成功後の所有権移譲か明示Abortまで解放しない。明示Abort、seal成功後の移譲失敗または移譲後のPlan pre-commit失敗は、現在ownerがRegistry取消後にDispositionを`Incomplete`へ確定して同じabort cleanupへ進む。Plan commit後の`FinalizedAndRegistered`ではRegistry取消とこのabort cleanupを呼ばず、既存Publication／Recovery／Cleanup経路だけがSession Ownership Leaseを最終解放する。

Draft、Registry、Trace の状態遷移は main thread の共通 Coordinator だけが反映する。Backend はこれらを変更しない。Draft の外部状態は `Pending / Staged / Dropped` を維持し、backend 内部状態を追加しない。Phase 0.11の`Staged`は当該Frameのencode／chunk append成功だけを表し、Run Artifactの公開可能性またはCaptureCompleteを単独では証明しない。形式非依存 Drop Reason は既存 enum へ append-only で追加し、既存 0～9 を再番号しない。Phase 0／0.1のFreeze順と意味は維持するが、Sessionの生成・解放はOwnership Lease APIへ移行する。Phase 0.11は新規受付停止、Backend `BeginDrain`、queued cancellation、pending encode／append完了、全Frame Completion回収、専用WorkerへのFinalize／Abandon要求、Context terminal resultのbounded poll、`Finalized`直接登録または`Abandoned`予約解放、`TryJoin`、全所有資源／予約ゼロ確認、main-thread terminal intent反映、残存Pendingのtimeout Drop、Disposition=`None`のままTrace seal、成功時のownership bundle移譲、Disposition=`None`のままPlan構築、Plan commit時のRegistry handle `PlanConsumed`化と`FinalizedAndRegistered`確定、pre-commit失敗／Abort時のRegistry取消と`Incomplete`確定、abort cleanupの順とする。全Accepted encode／appendのdrain完了前にFinalizeせず、`TryJoin`成功後に同WorkerへFinalizeを要求せず、`None`以外または未知DispositionでTrace seal／Plan構築へ進まず、Plan commit前に`FinalizedAndRegistered`へ確定しない。Join 前にbackend-owned Surface／buffer／未確定chunk payloadを外部からDisposeしない。
...
Phase 0.11はPhase 0.1で確定した非同期Capture境界を入口条件として使用し、Phase 0のPNG＋JSON Backendの成果物意味を変更しない。Session生成・解放APIだけはOwnership Leaseへ破壊的に移行する。複数Frame共有Artifactを既に表せる`CaptureArtifactFrameRelation`と、21.15で追加した最小Run Artifact拡張だけを使い、Capture FrameとFrameId／Traceの対応、Freeze／Drain／Join／Publication／Recovery／CaptureCompleteをBackend固有経路で迂回しない。指定GPU／Driver／SDKを持つhardware-qualified runnerのnominalだけは`BringUpCadenceTickCount=120`、`BringUpCadenceHz=30`、`SubmissionWindowDeadlineMs=4000`、`FinalizationDeadlineMs=30000`を固定する。Tick indexは`0..119`、予定時刻はmonotonicな開始時刻を`T0`として`T0 + index / 30 second`とし、各tickで提出を厳密に1回だけ試みてBackpressure時も同tickを再試行しない。tick 119の試行完了または`T0 + 4000 ms`到達の早い方で新規提出を停止し、その後Drain、全Frame回収、chunk確定／登録、`TryJoin`、資源ゼロ確認、Disposition=`None`でTrace Freeze、ownership bundle移譲、Disposition=`None`でPlan構築を行い、Plan commit時にだけ`FinalizedAndRegistered`へ確定してPublicationへ進む。Finalization期限は提出停止時点から測り、期限内に期待終端へ到達しなければscenario失敗としてCaptureだけをAbortする。この4秒は提出窓のhard boundであり、Finalizationを含む全Run時間を4秒と主張しない。一般CIではmock／fake completion sourceを使う短い決定論的lifecycle試験だけを必須とし、wall-clock 30fps、NVENC capabilityまたは120 Frame成功を要求しない。同一Runで提出を再開しない。

hardware-qualified `NvencNominalBringUpV1`は全120 tickについて提出試行、Accepted Work Token、encode Frame Completionを厳密に1対1で要求し、Frame Completionは全件`Succeeded, ProducedArtifactCount=0`とする。正常Drainで全Frame Completionを回収した後、Worker停止前に全120 FrameのAccess Unitをaccepted CaptureFrameId順に含むRun chunkを確定し、Context terminal回収とRegistry直接登録後に`TryJoin`する。chunkのpath、length、hash、TestRunIdと、正のCaptureFrameId 120件を重複なくaccepted順に持つFrame Relationを検査し、Contextの単一`Finalized` Result、Run専用Registry登録、`FinalizedAndRegistered` disposition、Publication、CaptureCompleteが30秒Finalization期限内に成功しなければPhase 0.11のnominal完了を認めない。受付拒否、Backpressure、Drop、encode／chunk確定失敗、件数不足をnominalの許容劣化にしない。この試験は対応hardwareのPhase承認／定期qualificationで実行し、GPUを持たない一般CIのmerge Gateにしない。

fault、Backpressure、Freeze／Drainの各scenarioは`FaultCadenceTickCount=16`、`FaultCadenceHz=30`、`FaultSubmissionWindowDeadlineMs=1000`、`FaultFinalizationDeadlineMs=10000`を上限とする。必要tick数をscenario開始前に`1..16`で宣言し、index `0..DeclaredTickCount-1`を`T0 + index / 30 second`で各1回だけ試みる。宣言tick末尾または`T0 + 1000 ms`の早い方で提出を止め、提出停止後10秒以内に期待Dispositionへ到達させる。事前宣言したFault Injectionと期待Dispositionに一致するAccepted／Frame Completion件数不足またはRun chunk `Abandoned`だけを許し、期待値にない不足、期限後の追加提出、同tick再試行、fault解除後の窓延長は禁止する。

再起動Recovery scenarioは旧processの提出停止時刻またはmonotonic clockを永続化・継承せず、新processがOS lockを取得して対象RunのRecovery entrypoint `BeginRecovery`へ入った瞬間を`RecoveryT0`として同processのmonotonic clockで固定する。`RecoveryFinalizationDeadlineMs=10000`以内に期待するRecovery dispositionへ到達できなければscenario失敗とし、期限延長、同一Runの再提出または自動再試行を行わない。確定済みchunkがPlanへ入っているscenarioだけPublication／CaptureCompleteを要求し、書込み中chunkだけを残してcrashしたscenarioでは`.partial`の無視または破棄とCapture Incompleteを正しい終端とする。processの停止から新process起動、初期化、`BeginRecovery`到達までの時間は外部Harnessが別の診断値として記録するが、この10秒の合否、Capture Run Manifestまたは永続Recovery schemaへ混入させない。fault系とRecoveryは30fps持続性能を再証明せず、状態遷移、所有権、Drain／Recovery終端だけを検査する。

`NvencBringUpProfileV1`はWindows／NVIDIA／D3D11、SDR／sRGB、左眼、30fps Capture cadence、`width=1280`、`height=720`を固定し、Capture EnvelopeとRun ManifestへProfile IDを保存する。Run開始時に`CaptureFrameProfile.ImageRect`が厳密に1280×720であることを検査する。NVENCへ提出する専用RenderTextureはImageRectと同じ全extent、`x=0`、`y=0`、`PixelLayout=RGBA8`、`GraphicsFormat.R8G8B8A8_SRGB`、MSAAなし、mipmapなし、row 0が表示画像上端のtop-left orientationとする。元Textureが別SubRect、Texture Arrayまたはbottom-left orientationなら、提出前のGPU Passでcrop／上下反転とRGBA8 sRGBからBT.709 limited-range NV12への変換を完了し、D3D11 GPU TextureとしてNVENCへ渡す。NVENC Backendは`UnityRenderTextureReadbackDispatcher`その他のPNG用RGBA32 CPU readbackを使用せず、圧縮後のbitstream bytesだけをCPUへ回収する。Backend内ではresize、追加crop、orientation推測を行わずalphaを無視する。1280×720以外の寸法、異なるFormat／Layout／orientation、Dynamic Resolution、ImageRectまたは寸法のRun中変更では新Encoder Sessionへ再構成せず、新規受付を止めてCaptureだけをFail Fastし、ゲームと既存の確定済みArtifactを維持する。

`NvencBringUpProfileV1`は`NvencWorkSlotCount=8`、`NvencInputSurfaceSlotCount=4`、`NvencFrameCompletionQueueCapacity=8`を固定し、Run開始前にWork Slot領域、NVENC Input Surface Pool、SPSC Completion Queueを生成する。各Input slotのD3D11 Texture／NVENC登録handle／所有状態と、各Work Slotのtoken generation／Completion credit／所有権状態を固定長領域へ保持する。`ICaptureEvidenceSession.TrySubmit`が受理した時点で、呼出側の`CaptureSurfaceLease`所有権はBackendへ移る。BackendはSource Textureを読むcrop／flip／RGBA→NV12 GPU変換を登録しただけではLeaseを解放しない。GPU queue上で当該変換がSource Textureを参照し終えたことを示す`SourceReadCompleted` Fence／Query等の非同期完了証拠を取得した後だけ、Backend内部の軽量Completion処理がLeaseを元Surface Poolへ返却またはDisposeする。これはSource所有権だけの解放であり、この時点では`CaptureFrameCompletion`を発行しない。Main／Render Threadは証拠を待たず、未完了Leaseを元Surface Poolへ返さない。

`NvencCaptureEvidenceBackend`は`MaximumArtifactCountPerSubmission=0`とし、Coordinatorから単一`NvencRunChunkContext`を受け取る。Run開始時にContext、Registry容量1件、writer、固定Work Slot 8件、NVENC Input Slot 4件、固定Frame Completion Queue 8件を一括確保できなければ新規Frameを受け付けず、取得済み資源とRegistry予約を逆順で全返却する。`TrySubmit`は空Work Slot、NVENC Input Slot、Frame Completion creditその他必要資源をAccepted前に一括予約できた場合だけ`Accepted`を返し、容量不足、`NotAccepting`または例外では内部予約を残さずSurface所有権を受け取らず無効tokenを返す。Source Surfaceの解放とNVENC Input Slotの再利用は別lifecycleである。Source Read完了後もNV12を保持するInput Slotは占有を継続し、NVENC encode完了証拠を取得して圧縮bitstreamの所有権をContextのwriterへ移した後だけ再利用する。Frame WorkはArtifact Descriptor、content hash、Artifact Completionを生成しない。取消、encode失敗、Drain時もSource Read完了前のLeaseとNVENC完了前のInput Slotを早期返却せず、device loss等で通常証拠を得られない場合は再利用可能Poolへ戻さず失敗終端してBackend破棄時に隔離解放する。

NVENC完了回収、bitstream ownership取得、accepted順への整列、chunk append、streaming hash／ByteLength更新、Frame Completion生成は単一の専用Workerへ直列化する。完了が逆順の場合は固定Work Slot内だけに保持し、先行Frame待ちで全Slotが埋まれば後続受付を`Backpressured`とするHead-of-line blockingを許容する。encodeとappend ownership移転が成功したFrameは`CaptureFrameCompletion(Status=Succeeded, ProducedArtifactCount=0)`、encode失敗、取消、null／空出力、Access Unit上限超過、chunk byte上限超過またはappend失敗では`Failed`または`Cancelled, ProducedArtifactCount=0`を予約済みFrame Completion creditへexactly once公開し、失敗時はContextを`Abandoned`へ固定して新規受付を停止する。Frame Completionには固定配列SPSC Queueを使い、producerはこの単一Worker、consumerはMain Threadとする。Queue操作は短いlockまたは同等のrelease／acquire同期を許可し、Main Threadはlock取得を待ち続けず取得できないTickでは次回pollへ送る。Accepted前にcreditを予約するため、Accepted後のenqueueは容量不足で失敗しない。Work Slotは対応Completionが回収され、全Source／Input／Access Unit所有権が解放またはwriterへ移転済みになった後だけ再利用する。Phase 0.1 PNG Queue実装そのものはcross-thread転用せず変更しない。lock-free Cell、fieldごとのbarrier、読取り前後のgeneration二重照合または全timingの競合網羅をPhase 0.11の成果物にしない。

Main ThreadとRender ThreadではNVENC Completion、GPU Fence、Texture空き、bitstream lock／取得、hash、file write、close／renameを待たない。Render Thread／Native Plugin callbackはcrop／flip／色変換を行うGPU workとNVENC submitに必要なbounded command／handle登録だけを行って戻り、callback内でblocking encode、poll loop、sleep、device-context待機またはI/Oを行わない。完了証拠のpoll／通知待ち、待機可能なbitstream取得、content hash、chunk staging append／finalizationは専用Worker側で行う。4 Input Slotまたはcaller側Source Surface Poolがすべて占有中ならMain／Render Threadを待たせず`Backpressured`を返す。nominalでBackpressureが1件でも発生すれば失敗とし、専用fault scenarioでは期待Dispositionとして検査する。

crop、orientation補正、RGBA→NV12は可能な範囲で同一のversion付きGPU conversion passへまとめ、CPU roundtripを禁止する。API／format制約で追加GPU copyが必要な場合はPass数、copy数、Texture遷移を固定ProfileとProfilerへ記録するが、zero-copy方式比較や複数実装の最適化をPhase 0.11へ持ち込まない。構造テストでは`NvencCaptureEvidenceBackend`が`UnityRenderTextureReadbackDispatcher`を参照しないこと、Main／Render Thread経路から待機可能APIとArtifact Storeを呼ばないことを検査し、ProfilerでNative callback時間、Source Surface／Input Slot占有、Backpressure数を取得する。Phase 0.11の必須回帰は次の10代表caseに限定する。(1)正常120 Accepted Frameが1 chunkとして確定、登録、Trace Freeze、ownership bundle移譲、Plan commit、Publicationされる、(2)Work／Input／Completion容量不足で所有権を移さずBackpressure、(3)Source GPU read完了前のLease再利用禁止、(4)encodeまたはappend失敗でchunk全体をAbandoned、Trace Freeze後もIncompleteでPlan／CaptureCompleteへ非遷移、(5)close、rename、Registry登録、所有権移譲またはPlan pre-commit失敗で非Publication、(6)pending Workを含むDrainがFrame回収、Finalize／Abandon、terminal回収、TryJoin、資源確認、Trace Freeze、移譲、Plan commit／Abortの順で終端、(7)crash後の`.partial`またはPlan未登録orphanがPlan／CaptureCompleteへ非混入、(8)Finalize対Abandonの一方だけが終端、(9)Main／Render Threadから待機可能API、hash、file I/Oを非呼出、(10)確定chunkをclean Decoder process 1回でdecodeする。容量不足caseは反復してもWork Slot、Input Slot、Completion credit、Context Registry予約が減らずcallerがSurfaceを保持することを確認する。遅延caseはSource Read証拠前にLeaseを再利用せず、Source Readだけの完了ではLeaseだけを解放してInput SlotとFrame Completionを未完了とする。成功時は各Frameが`Succeeded/count=0`を厳密に1件返し、全Frame回収後かつTryJoin前だけContextが`Finalized` Resultを1件生成する。失敗／取消Frameまたは0 Accepted Frameでは`Abandoned`として予約を1回だけ解放し、Resultを生成しない。Frameが全てStagedでもContextが`Abandoned`またはRegistry未登録ならArtifact 0件のPlanを構築せず、Publication ReceiptとCaptureCompleteを発行しない。Dispositionの`None=0`だけがTrace sealと、正確なownership bundleを伴うPlan構築を開始でき、seal再試行、移譲およびPlan pre-commit中は`None`を維持し、Plan commit時だけRegistry handleを`PlanConsumed`にして`FinalizedAndRegistered`へ確定することを検査する。明示Abort、移譲失敗またはPlan pre-commit失敗では現在ownerが正確なRegistry Entryを取り消して`Incomplete`へ確定し、commit後の失敗ではEntryを取り消さずRecoveryへ送ることを検査する。Registry取消はexact handleだけを除去し、別Entry、`PlanConsumed`済みEntryまたは二重取消へ副作用を与えない。所有権移譲失敗時は送信側がContext／Session Ownership Lease／Lock Identity／未消費Freeze Receipt／Registry handleを全て保持し、成功時は旧Session Ownership Leaseと旧Registry handleを失効させ、受信側の次generationだけがDispose／取消できることを検査する。通常Session alias、非所有Lock Identity、foreign／stale Lease、二重DisposeからOS lockを解放できないこと、移譲成功後・Plan確定前失敗では受信側がEntry取消、cleanup、Lease解放を完了することを検査する。Plan commit marker確定後のpost-commit診断失敗をpre-commit Abortへ誤分類しない。`Incomplete`はcleanup成功／失敗の双方でContext／Backend／現在のSession Ownership Leaseを一度だけ解放し、残存rootを次回Recoveryが自動公開しないことを確認する。初期化失敗、SPSC Queue wraparound、Frame Token重複、Contextの二重終端、Drain後のWork Slot／Registry予約／Lease／Input Slot／Access Unit buffer／writer handleの欠落・永久占有を短い単体試験で検査する。foreign Run Token、Producer／writer再結合、汎用Receipt偽造の直積、Descriptor／Relation／path差替えの直積、cardinality 0／1／2／変更、inline Cell全timing barrier試験を削除し、不具合再現または実測で必要になった試験だけ追加する。複数Run Artifactまたは2番目のchunkはPhase 0.11の構造上生成できず、Phase 4.8の別versionへ送る。

Plan境界の回帰では、Phase 0.11のpre-commit失敗後に`publication.plan.nvenc-precommit.tmp`削除も失敗させて再起動し、canonical bytes、正しいRun相関および対応chunkが存在しても`publication.plan`へ昇格せず隔離／破棄されることを確認する。同じsuiteでPhase 0／0.1のcanonical `publication.plan.tmp`だけが残るcaseは従来どおり昇格できることを確認する。Phase 0.11専用rename直後に例外を注入し、bounded再読込から`Committed`へ収束してRegistry Entry取消／staging削除を行わないこと、directory metadata flushをPhase 0.11成功条件にせずPhase 0／0.1では引き続き必須とすることを検査する。完成`publication.plan`と`publication.plan.nvenc-precommit.tmp`を同時に配置した再起動caseは`PublicationRecoveryCollision`へ固定し、final、tmp、chunkその他Run root内の全fileの存在、長さ、content hashが検査前後で不変であり、Plan／Artifact公開とCaptureCompleteが発生しないことを確認する。

Run開始時はNVENC capability queryでH.264 High Profile、NV12入力、固定寸法、必要なD3D11／非同期Encode機能を問い合わせ、要求設定によるEncoder Session初期化の成功を必須とする。アプリ内でAnnex AのMaxFS／MaxMBPS／MaxBR／MaxCPBを再計算せず、対応可否はcapability queryと初期化結果を正本とする。RGBA8 sRGBからBT.709 limited-range NV12への変換はversion付き固定GPU Passで行うが、係数、量子化丸め、chroma sample location、H.264 VUIの個別bit値をPhase 0.11のProduction受入契約にしない。変換Profile IDとVersionだけをRun Manifestへ記録し、色とorientationの妥当性は後述のDecoder結合テストで確認する。
...
正常Freezeでは新規受付停止後にAccepted WorkをDrainし、全pending encode／appendを完了して全Frame Completionを回収する。この時点では単一WorkerをJoinせず、Run chunkを`Finalized`にできる最小条件として、(1) AcceptedかつSucceededのFrameが1件以上、(2) Access Unit append件数が1件以上、(3) Succeeded Frame数、append済みAccess Unit数、Frame Relation件数が一致し、同じaccepted順であること、(4) 全Accepted FrameがSucceededであること、を検査する。0 Accepted Frame、0 Access Unit、件数／順序不一致またはいずれかのFrame失敗では空chunkを確定せず同Worker上で`Abandoned`へ進む。

最小条件を満たした場合だけ、Coordinatorは停止前の専用WorkerへContextのFinalizeを要求する。workerは同じ直列化境界でappendを停止し、appendごとに更新済みのchecked累積ByteLengthと単一streaming content hashを確定してhandleをcloseし、確定staging pathへ非上書きrenameした後だけ、Context内部状態からDescriptor／Relation／pathを持つ`NvencChunkFinalizationResult`を構築して`Finalized`を公開する。hashのためにfileを再読込せず、`Flush(true)`を呼ばず、close後にhandleを再openしない。Main Thread Coordinatorはterminal resultをbounded pollし、自身が所有するContextとResultをPhase 0.11専用Registry経路へexactly once渡してPlan用Descriptor／Relationを登録し、正確なRegistry Entry handleを保持する。Context terminal回収と登録または予約解放が終わった後にだけ`TryJoin`し、成功後に同Workerへ処理を要求しない。全Frame Completion回収前のFinalize要求またはResult登録を禁止する。Joinと資源ゼロ確認まで成功した候補はDisposition=`None`のままTrace sealへ進み、成功後にexact Context／Session Ownership Lease／Lock Identity／Freeze Receipt／Registry Entry handleを既存Publication Coordinatorへall-or-noneで移譲する。移譲後もDisposition=`None`とRegistry Entryの取消可能状態を維持し、受信側だけがownership bundleを使ってPlanを構築する。Plan commit時にhandleを`PlanConsumed`へ終端化し、同じcommit結果の反映で`FinalizedAndRegistered`を確定してPublication Receiptを発行する。

encode、append、hash、close、rename、Result生成、Registry登録、Joinまたは資源ゼロ確認の失敗・取消により安全にTrace sealできない場合は明示Abortとし、登録済みEntryを現在ownerのexact handleで取り消して予約を解決した後に`Incomplete`へ確定する。Contextが`Abandoned`でもJoinと資源解放を完了してTrace sealできる場合は`None`のままTraceをFrozen化した後に`Incomplete`へ確定する。成功候補のTrace sealが失敗した場合は`None`のまま再試行するか、明示Abortして同じ取消／Incomplete経路へ進む。Trace seal成功後の所有権移譲失敗では元Receiptと全所有権を送信側に維持し、Registry取消後に`Incomplete`へ確定する。移譲成功後のPlan pre-commit失敗では受信側が次generationのRegistry handleでEntryを取り消し、Session Ownership Leaseを解放して`Incomplete`へ確定する。Plan commit後の失敗ではhandleが`PlanConsumed`であるため取消／file削除せず、同じOwnership Leaseを既存Recovery／再試行へ維持する。Finalize前の失敗ではContextを`Abandoned`へexactly onceで固定し、Finalize後の失敗ではContextを`Finalized`のまま維持する。pre-commit失敗経路ではPublication Receipt、Plan Entry、Capture IndexまたはCaptureCompleteを生成せず、ゲーム、既に確定した別Artifact、既に公開したFrame Completionをrollbackしない。したがってFrame Completion成功は当該Frameのencode／append成功、Plan commit後の`FinalizedAndRegistered`だけがchunkの通常Publication／Recovery可能を表す。Trace enqueue／seal、移譲またはpre-commit失敗はRun Incompleteになり得るがFrame CompletionまたはContext outcomeを再発行せず、Frozen Traceへ事後理由を追記しない。

RecoveryはProduction H.264 parserを持たず、Planへ登録済みの確定chunkについてDescriptorのcanonical path、ByteLength、content hash、TestRunId、ArtifactId、Frame Relationをboundedに再検査する。保存済み確定bytesのlength／hash不一致、path／Run／Frame相関不一致では対象Artifactを変更せずRecovery collisionとして停止する。`.partial`、Planに未登録のstaging chunk、確定rename前に停止したchunkは未確定であり、OS lock下で安全に無視または全体を破棄する。NAL走査、安全位置へのtruncate、末尾救済、部分hash、部分Relation復元、部分公開を一切行わない。未確定chunk喪失は当該CaptureをIncompleteにするが、ゲームRun全体または既に確定した別Artifactの破損とは扱わない。Recovery時にstart code、SPS／PPS、Slice、Level、VUIを解析せず、raw bytesをcanonicalize／再serializeしない。OS crash／電源断に対する完全durabilityをPhase 0.11で保証しない。

Phase 0.11 RecoveryのPlan authorityは完成名`publication.plan`だけとする。`publication.plan.nvenc-precommit.tmp`だけが残るrootは、明示Abort、pre-commit失敗またはcrashのいずれであるかを推定せず、同tmpをPlanとしてparse／昇格／部分利用しない。OS lock、no-follow、固定path、Run root相関を検証して隔離または全体破棄し、削除失敗時も自動公開せずIncomplete rootとして残す。完成`publication.plan`とNVENC専用tmpが同時に存在する場合は、どちらの内容がcanonicalまたは相互一致するかにかかわらず`PublicationRecoveryCollision`としてfail closedに停止する。final、tmp、chunkまたは他のRun root内fileを削除、rename、上書き、公開せず、CaptureCompleteを発行しない。再起動後はprocess-localな`NvencRunEvidenceDisposition`を復元・推定せず、Recovery outcomeだけをcollisionへ固定する。完成`publication.plan`だけが存在する場合に限り、final Planと確定chunkの通常bounded検証へ進む。Phase 0／0.1の`publication.plan.tmp`昇格規則へこの制限を波及させない。

Phase 0.11のDecoder結合テストはProduction Captureとは独立した`NvencChunkDecodeSmokeProfileV1`を使う。DecoderはFFmpeg 9.0.1の固定Portable Bundleとし、実行file／packageのSHA-256、起動option、OS、NVENC SDK、GPU、Driverをテスト結果へ記録する。Bundle treeのcanonical index、環境変数schema、DLL探索の再実装、append-only外部Tool Profile基盤をPhase 0.11の成果物にしない。Test Runnerは検証済み絶対pathのDecoderを使い、確定Run chunk 1件につき新しいprocess／sessionと空の作業directoryを1つだけ作る。raw Annex B chunkだけを入力し、外部extradata、別Run Artifact、Decoder session／cacheを再利用しない。timeout、stdout／stderrのbounded回収、失敗時のprocess tree停止と作業directory回収はTest Runnerの安全要件として維持するが、これらをProduction schemaへ追加しない。

nominalの確定chunkをclean Decoder process 1回で先頭から末尾まで逐次decodeし、出力Frame数がFrame Relation件数と厳密に一致し、nominalでは120であること、全Frameが1280×720 RGBA8であることを確認する。Frame Relation列のordinal `i`とdecode出力ordinal `i`を対応させ、Work Token／CaptureFrameIdの取り違えがないことを、入力FixtureのFrame markerまたはテスト専用expected sequenceで確認する。Productionは各Frameのbyte offset、seekまたはrandom decodeを保証せず、Smoke Testもそれらを要求しない。確定chunkのpath、length、hash、Run相関とFrame Relationは全件検査するが、未確定chunkをDecoderへ入力しない。
