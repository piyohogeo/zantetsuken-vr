...
project=product.parent/'zantetsuken-assets-private/Working/D6T-cdb53ea/Project'
raw=product/'Logs/CharacterRequestMigration'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
paths=[
  "Runtime/PhysicsCut/CutWorldRoot.cs",
  "Runtime/PhysicsCut/PhysicsOwnerRegistry.cs",
  "Runtime/PhysicsCut/VpPreparedCharacterCut.cs",
  "Runtime/PhysicsCut/VpPreparedCharacterCut.Request.cs",
  "Tests/EditMode/PhysicsCut/PreparedCharacterColdTests.cs",
  "Tests/EditMode/PhysicsCut/PreparedCharacterRequestTests.cs",
  "Tests/PlayMode/PreparedCharacterColdPlayModeTests.cs",
  "Tests/PlayMode/PreparedCharacterRequestPlayModeTests.cs"
]
manifest=[]
...
for directory in sorted(raw.glob('*-*')):
    ...
    item['d6Cases']=sum('D6T_' in c.attrib['name'] for c in cases)
    ...
    runs.append(item)
summary=dict(baseProductCommit='cdb53ea0e47dbdd5f58a04eb396d7c50d27b040a',
    ...
    publicColdPreparationApiAdded=True,hotAdapterAdded=True,performanceMeasured=False,playerVerified=False,productUnityLaunched=False)
if '--check' in sys.argv:
    for prefix,total,d6 in [('focused-',16,16),('full-',3871,16),('play-',3,3),('related-',10,3)]:
    ...
    print('D6 synchronous request: current candidate, fixtures and evidence match; focused 16, full 3871, PlayMode 3 and related 10 passed.')
