...
An isolated upload sample measures 101 repeated vertex/index transfer calls after 10 warmups, with no image readback/rendering/file IO in the timing window. It includes the existing index read-lease boundary. It measures CPU API time and managed allocation only: neither total Main frame time, driver/GPU completion, WDDM residency nor a 32B performance comparison. Re-uploading all committed vertices is a probe workload, not the production incremental-upload policy. Capacity/payload byte counts are logical quantities only.
