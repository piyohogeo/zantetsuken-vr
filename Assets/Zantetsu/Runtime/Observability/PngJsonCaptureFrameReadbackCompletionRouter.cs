...
namespace Zantetsu.Observability
{
    ...
    /// </para>
    /// </remarks>
    public sealed class PngJsonCaptureFrameReadbackCompletionRouter
    {
        private readonly UnityRenderTextureReadbackDispatcher _dispatcher;
        private readonly CaptureFrameTraceObserver _observer;
        private readonly IPngJsonCaptureFrameEncodeService _encodeService;
        private readonly PngJsonCaptureFrameEncodeCompletionCoordinator _encodeCompletionCoordinator;

        public PngJsonCaptureFrameReadbackCompletionRouter(
            ...
            CaptureFrameTraceObserver observer)
        {
            ...
            _observer = observer;
            _encodeService = new PngJsonSynchronousCaptureFrameEncodeService(1);
            _encodeCompletionCoordinator = new PngJsonCaptureFrameEncodeCompletionCoordinator(_encodeService, observer);
        }
            ...
            out bool released)
        {
            ...
            if (submitStatus != PngJsonCaptureFrameEncodeSubmitStatus.Accepted)
            {
                payload.ReleaseByCaller();
                released = payload.ReleaseSucceeded;
            ...
            if (!_encodeService.TryCollect(out PngJsonCaptureFrameEncodeCompletion completion))
            {
            ...
            try
            {
                PngJsonCaptureFrameEncodeApplyResult applied = _encodeCompletionCoordinator.Apply(completion);
                released = payload.ReleaseSucceeded;
