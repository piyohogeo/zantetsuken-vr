...
namespace Zantetsu.Observability
{
    ...
    /// <see cref="CanonicalBytesToken"/> binding the authoritative plan to the
    /// supplied byte array by reference identity, so an arbitrary byte array
    /// cannot be substituted. It takes ownership of the caller's
    ...
    {
        /// <summary>
        /// Proof that a byte array is the canonical serialization of a specific
        /// authoritative plan. The token binds the plan and the generated byte
        /// array by reference identity, so a caller cannot substitute a
        /// different byte array for the canonical bytes while keeping a single
        /// serialization in the factory.
        /// </summary>
        internal sealed class CanonicalBytesToken
        {
            ...
            internal bool IsIssuedFor(CapturePublicationPlan plan, byte[] bytes)
            {
                return plan != null
                    && bytes != null
                    && ReferenceEquals(_plan, plan)
                    && ReferenceEquals(_bytes, bytes);
            }

            internal static CanonicalBytesToken Acquire(CapturePublicationPlan plan, byte[] bytes)
            {
                if (plan == null)
                {
                ...
                {
                    throw new ArgumentException("Canonical bytes must be present.", nameof(bytes));
                }

                return new CanonicalBytesToken(plan, bytes);
            }
        }
        ...
        internal CaptureRunCaptureIndexCommitOperation(
            ...
            int stepIndex,
            CanonicalBytesToken canonicalBytesToken,
            ref byte[] canonicalBytes)
        {
            ...
            if (!canonicalBytesToken.IsIssuedFor(authoritativePlan, canonicalBytes))
            {
                throw new ArgumentException("Canonical bytes token must be issued for the authoritative plan and the supplied bytes.", nameof(canonicalBytesToken));
            }
