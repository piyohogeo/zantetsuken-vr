...
{
    /// <summary>
    /// Fixed-capacity logical pool of eight Frame Work Slots for the Phase 0.11
    /// NVENC capture path. It reserves and releases slots only; it owns no
    /// Texture, NVENC handle, Completion Event, queue, or other native resource.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Capacity is fixed at <see cref="NvencBringUpProfileV1.WorkSlotCount"/>
    /// (always eight) and every backing array is allocated once in the
    /// constructor. <see cref="TryRent"/> and <see cref="TryReturn"/> perform no
    /// managed allocation, no waiting, and no I/O, and a full pool fails
    /// immediately with a <c>default</c> lease instead of waiting.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    internal sealed class NvencCaptureWorkSlotPool
    {
        ...
        internal NvencCaptureWorkSlotPool(NvencCaptureProcessState processState)
        {
        ...
        internal int Capacity => NvencBringUpProfileV1.WorkSlotCount;

        internal int OccupiedCount => _rentedCount;

        internal bool TryRent(out NvencCaptureWorkSlotLease lease)
        {
            ...
            for (int i = 0; i < _rented.Length; i++)
            {
                if (!_rented[i] && !_retired[i])
                {
                    _rented[i] = true;
                    _rentedCount++;
                    lease = new NvencCaptureWorkSlotLease(_ownerToken, i, _generations[i]);
                    return true;
                }
            }
        ...
        internal bool TryReturn(in NvencCaptureWorkSlotLease lease)
        {
        ...
        internal bool IsActive(in NvencCaptureWorkSlotLease lease)
        {
            if (!lease.IsValid || lease.OwnerToken != _ownerToken)
            {
                return false;
