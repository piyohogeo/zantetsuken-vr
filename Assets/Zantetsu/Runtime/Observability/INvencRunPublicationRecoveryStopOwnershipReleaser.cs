...
{
    /// <summary>
    /// Synchronous release boundary for a Run whose publication recovery
    /// stopped: one call is one attempt to release the exact Session Ownership
    /// Lease the operation carries.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// This boundary belongs to the stopping publication recovery graph alone;
    /// it is deliberately not merged with the CaptureComplete recovery release,
    /// whose authority graph is a different one. An implementation touches no
    /// file, re-inspects nothing, cleans up nothing, and leaves the Registry,
    /// the disposition, the Service, and every process state alone, owning no
    /// thread, queue, task, or wait primitive.
    /// </para>
    /// </remarks>
    internal interface INvencRunPublicationRecoveryStopOwnershipReleaser
    {
        NvencRunPublicationRecoveryStopOwnershipReleaseReceipt Release(
