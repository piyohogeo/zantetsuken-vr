...
{
    /// <summary>
    /// The single place that decides whether one Phase 0.11 NVENC recovery
    /// ownership release attempt may be started, shared by the release boundary
    /// and its synchronous execution coordinator so the two can never
    /// disagree.
    /// </summary>
    /// <remarks>
    ...
    /// inadmissible before the first attempt are all refused. This type holds
    /// no state, keeps no retry counter or attempt latch, reads no process
    /// state, and changes nothing.
    /// </remarks>
    internal static class NvencRunCaptureCompleteRecoveryOwnershipReleaseAdmission
    {
        /// <summary>
