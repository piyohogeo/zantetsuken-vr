...
namespace Zantetsu.Core.Tests
{
    public class PngJsonCaptureFrozenRunPublicationPlanBindingContractTests
    {
        ...
        public void Binding_ForwardsExactReferences()
        {
            ...
            Assert.That(ReferenceEquals(binding.LockLease, frozen.LockLease), Is.True);
        ...
        {
            CaptureEvidenceFrozenRunPublicationResult frozen = MakeFrozenResult(MakeGenericPlan(3, new long[] { 1 }));
            frozen.RunSession.Dispose();
        ...
        [Test]
        public void Binding_SessionLeaseDispose_False()
        {
            PngJsonCaptureFrozenRunPublicationPlanBinding binding = MakeBinding(1);
            Assert.That(binding.IsValid, Is.True);

            binding.RunSession.Dispose();
            Assert.That(binding.IsValid, Is.False);
        }
