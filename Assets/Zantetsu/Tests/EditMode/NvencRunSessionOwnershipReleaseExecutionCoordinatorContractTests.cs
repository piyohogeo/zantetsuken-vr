...
{
    /// <summary>
    /// Contract tests for the Phase 0.11 Fresh NVENC Session Ownership Lease
    /// release synchronous boundary: the releaser interface, the immutable
    /// success-only receipt, and the single-call execution coordinator. Uses
    /// the fully published, CaptureComplete-ed, cleaned-up Run pipeline to mint
    /// a valid release operation, and a throw-once lock handle to produce a
    /// real partial release; no real GPU, NVENC, filesystem, sleep, or short
    /// negative wait is used.
    /// </summary>
    public class NvencRunSessionOwnershipReleaseExecutionCoordinatorContractTests
    {
        ...
        [Test]
        public void Constructor_NullReleaser_Rejected()
        {
            ArgumentNullException ex = Assert.Throws<ArgumentNullException>(
                () => new NvencRunSessionOwnershipReleaseExecutionCoordinator(null));
            Assert.That(ex.ParamName, Is.EqualTo("releaser"));
        }

        [Test]
        public void Execute_NullOperation_ArgumentNullException_ReleaserNotContacted()
        {
            FakeReleaser releaser = new FakeReleaser();
            NvencRunSessionOwnershipReleaseExecutionCoordinator coordinator =
        ...
        [Test]
        public void Execute_PoisonedBeforeFirstAttempt_Rejected_ReleaserNotContacted()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);

                // A first attempt requires admission validity, which a Poison
                // revokes. The lease is untouched and still fully retained, so
                // CanRelease alone must not let this through.
        ...
        [Test]
        public void Execute_AlreadyFullyReleased_Rejected_ReleaserNotContacted()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);
                ...
                NvencRunSessionOwnershipReleaseExecutionCoordinator coordinator =
                    new NvencRunSessionOwnershipReleaseExecutionCoordinator(releaser);
                Assert.That(coordinator.Execute(operation), Is.Not.Null);
                Assert.That(releaser.CallCount, Is.EqualTo(1));

                // A completed release admits no further attempt, and the
                // releaser is not contacted a second time.
        ...
        [Test]
        public void Execute_FirstAttempt_CallsReleaserOnce_ReturnsCorrelatedReceipt()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);
                ...
                NvencRunSessionOwnershipReleaseExecutionCoordinator coordinator =
                    new NvencRunSessionOwnershipReleaseExecutionCoordinator(releaser);
                Assert.That(ReferenceEquals(coordinator.Releaser, releaser), Is.True);

                NvencRunSessionOwnershipReleaseReceipt receipt = coordinator.Execute(operation);
                ...
                // The lock is gone and the receipt does not rest on the
                // operation's admission validity, which the release itself
                // necessarily revoked.
        ...
        [Test]
        public void Execute_PartialReleaseFailure_ThenRetrySucceeds()
        {
            using (Harness h = Harness.Create(throwingFirstRelease: true))
            {
                ...
                // The lease's first release fails after releasing one handle,
                // so the attempt propagates and issues no receipt.
                Assert.Throws<AggregateException>(() => coordinator.Execute(operation));
                Assert.That(releaser.CallCount, Is.EqualTo(1));

                // A partial release: no longer fully retained, disposal not
                // complete, and still releasable. Admission validity is
                // correctly false and must not block the retry.
                Assert.That(operation.OwnershipLease.IsCreated, Is.False);
                ...
                // The same operation retries and succeeds.
                NvencRunSessionOwnershipReleaseReceipt receipt = coordinator.Execute(operation);
        ...
        [Test]
        public void Execute_ReleaserThrows_PropagatesSameReference_NoRetryNoReceipt()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);

                InvalidOperationException failure = new InvalidOperationException("release failed");
                ...
                // Nothing was released, retried, or rolled back, and no receipt
                // was fabricated for the failure.
                Assert.That(operation.OwnershipLease.IsCreated, Is.True);
                Assert.That(operation.OwnershipLease.IsReleaseComplete, Is.False);
                Assert.That(operation.CanRelease, Is.True);
            }
        ...
        public void Execute_NullReceipt_Rejected()
        {
            AssertReceiptRejected(
                ...
                skipRelease: false,
                "a null receipt must be rejected");
        }

        [Test]
        public void Execute_ForeignReleaserReceipt_Rejected()
        {
            // The release really happened, so the receipt is valid in its own
            // right and only the foreign releaser can reject it.
            AssertReceiptRejected(
                (releaser, operation) =>
                {
                    NvencRunSessionOwnershipReleaseReceipt forged =
                        NvencRunSessionOwnershipReleaseReceipt.Create(new FakeReleaser(), operation);
                    Assert.That(forged.IsValid, Is.True);
                    Assert.That(ReferenceEquals(forged.Operation, operation), Is.True,
                        "the forged receipt must name the executed operation.");
                    Assert.That(ReferenceEquals(forged.Releaser, releaser), Is.False);
                    return forged;
                },
                skipRelease: false,
                "a receipt issued to a foreign releaser must be rejected");
        }

        [Test]
        public void Execute_ForeignOperationReceipt_Rejected()
        {
            using (Harness source = Harness.Create())
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation foreign = PrepareRelease(source);
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);

                // The foreign operation's own lease is released first so a
                // receipt can legitimately exist for it.
                FakeReleaser foreignReleaser = new FakeReleaser();
                Assert.That(new NvencRunSessionOwnershipReleaseExecutionCoordinator(foreignReleaser)
                    .Execute(foreign), Is.Not.Null);

                FakeReleaser releaser = new FakeReleaser
                {
                    OverrideFactory = (executing, executed) =>
                    {
                        // Names the executing releaser and is valid in its own
                        // right, so the foreign operation is the only thing that
                        // can reject it.
                        NvencRunSessionOwnershipReleaseReceipt forged =
                            NvencRunSessionOwnershipReleaseReceipt.Create(executing, foreign);
                        Assert.That(ReferenceEquals(forged.Releaser, executing), Is.True,
                            "the forged receipt must name the executing releaser.");
                        Assert.That(forged.IsValid, Is.True);
                        Assert.That(ReferenceEquals(forged.Operation, executed), Is.False);
                        return forged;
                    },
                };
        ...
        [Test]
        public void Execute_NotReleasedReceipt_Rejected()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);

                // A receipt built by the private constructor while the lease is
                // still held: the exact releaser and the exact operation, so the
                // missing release is the only thing that can reject it.
                FakeReleaser releaser = new FakeReleaser();
                NvencRunSessionOwnershipReleaseReceipt forged = MakeReceipt(releaser, operation);
                Assert.That(ReferenceEquals(forged.Releaser, releaser), Is.True,
                    "the forged receipt must name the executing releaser.");
                Assert.That(ReferenceEquals(forged.Operation, operation), Is.True,
                    "the forged receipt must name the executed operation.");
                Assert.That(forged.IsValid, Is.False);

                releaser.SkipRelease = true;
                releaser.OverrideFactory = (executing, executed) => forged;
        ...
        [Test]
        public void Receipt_SealedInternal_TwoReadonlyReferenceFields()
        {
            Type type = typeof(NvencRunSessionOwnershipReleaseReceipt);
            ...
            Assert.That(
                ...
                Is.EquivalentTo(new[]
                {
                    typeof(INvencRunSessionOwnershipReleaser),
                    typeof(NvencRunSessionOwnershipReleaseOperation),
                }));
        ...
        [Test]
        public void ExecutionCoordinator_SealedInternal_OneReadonlyReleaserField()
        {
            Type type = typeof(NvencRunSessionOwnershipReleaseExecutionCoordinator);
            Assert.That(type.IsSealed, Is.True);
            Assert.That(type.IsPublic, Is.False);
            Assert.That(typeof(IDisposable).IsAssignableFrom(type), Is.False);
            ...
            Assert.That(fields[0].FieldType, Is.EqualTo(typeof(INvencRunSessionOwnershipReleaser)));
            ...
            Assert.That(
                ...
                Is.Empty,
                "the execution coordinator must hold no mutable static state.");
        }

        // ---- Run state ----

        [Test]
        public void Execute_LeavesDispositionRegistryCleanupResultAndServiceUnchanged()
        {
            using (Harness h = Harness.Create())
            {
                NvencRunSessionOwnershipReleaseOperation operation = PrepareRelease(h);
                ...
                Assert.That(new NvencRunSessionOwnershipReleaseExecutionCoordinator(new FakeReleaser())
                ...
                // No earlier collaborator is contacted again and the retained
                // cleanup result is the same reference.
        ...
        /// <summary>
        /// Returns the forged receipt from the exact releaser that executes the
        /// exact operation, so the part under test is the only thing that can
        /// reject it. A helper using a fresh releaser or a fresh operation would
        /// be rejected by the reference checks first and would pass even if the
        /// release verification regressed.
        /// </summary>
        private static void AssertReceiptRejected(
            Func<FakeReleaser, NvencRunSessionOwnershipReleaseOperation,
            ...
            using (Harness h = Harness.Create())
            {
                ...
                Assert.Throws<InvalidOperationException>(
                    () => new NvencRunSessionOwnershipReleaseExecutionCoordinator(releaser)
                        .Execute(operation),
                    message);
                Assert.That(releaser.CallCount, Is.EqualTo(1), message);
            }
        }

        private static NvencRunSessionOwnershipReleaseReceipt MakeReceipt(
            INvencRunSessionOwnershipReleaser releaser,
            NvencRunSessionOwnershipReleaseOperation operation)
        {
            ConstructorInfo ctor = typeof(NvencRunSessionOwnershipReleaseReceipt).GetConstructor(
            ...
            return (NvencRunSessionOwnershipReleaseReceipt)ctor.Invoke(
                new object[] { releaser, operation });
        }

        /// <summary>
        /// Releases the operation's exact lease exactly once per call and
        /// issues the ordinary receipt, unless it is told to throw or to return
        /// a forged one.
        /// </summary>
        private sealed class FakeReleaser : INvencRunSessionOwnershipReleaser
        {
            private int _callCount;

            internal Exception ExceptionToThrow;
            internal NvencRunSessionOwnershipReleaseOperation LastOperation;

            /// <summary>
            /// Builds the receipt to return instead of the ordinary one. It is
            /// invoked after the release, so a forged receipt can be built in
            /// exactly the state a genuine one would be and the part under test
            /// is the only thing that can reject it.
            /// </summary>
            internal Func<FakeReleaser, NvencRunSessionOwnershipReleaseOperation,
                NvencRunSessionOwnershipReleaseReceipt> OverrideFactory;

            /// <summary>
            /// Skips the release, so a receipt can be forged for a lease that is
            /// still held.
            /// </summary>
            internal bool SkipRelease;
        ...
        /// As the first handle of a lease it produces the ordinary API's
        /// partial release: the second handle is released, the disposal throws,
        /// and the Ownership Lease is left no longer fully retained but still
        /// releasable.
        /// </summary>
