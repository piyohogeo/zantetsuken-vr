...
namespace Zantetsu.Rendering.Tests
{
    ...
    public class VpGeometryReferenceTableTests
    {
        ...
        public void AStorage_TakesOnlyOneReferenceTable()
        {
            Mesh quad = Quad();
            using (var storage = new VpCpuGeometryStorage(16, 16, 2, 8, 16, Allocator.Persistent))
            using (var otherStorage = new VpCpuGeometryStorage(16, 16, 2, 8, 16, Allocator.Persistent))
            {
        ...
        public void ARetiredGeometryWithALease_KeepsItsIndicesRetiringUntilTheLeaseReturnsAndThenTheirSpaceIsReused()
        {
            ...
            Mesh quad = Quad();
            using (var storage = new VpCpuGeometryStorage(16, QuadIndices, 2, 8, 16, Allocator.Persistent))
            {
        ...
        {
            using (var storage = new VpCpuGeometryStorage(16, 32, 4, 8, 16, Allocator.Persistent))
            using (var other = new VpCpuGeometryStorage(16, 32, 4, 8, 16, Allocator.Persistent))
            {
                ...
                foreach ((VpStoredGeometry candidate, string label) in new[]
                         {
                             ...
                             (new VpStoredGeometry(storage.VertexCount - 1, 2, unregistered.indexRange, false, 0, 0, 0, unregistered.blockStart, unregistered.blockCount), "uncommitted vertices"),
                             (new VpStoredGeometry(-1, QuadVertices, unregistered.indexRange, false, 0, 0, 0, unregistered.blockStart, unregistered.blockCount), "negative vertex start"),
                             (new VpStoredGeometry(unregistered.vertexStart, unregistered.vertexCount, unregistered.indexRange, false, 0, 0, storage.SubmeshCount + 1, unregistered.blockStart, unregistered.blockCount), "submeshes beyond the committed ones"),
                             (new VpStoredGeometry(unregistered.vertexStart, unregistered.vertexCount, unregistered.indexRange, false, 1, unregistered.submeshStart, unregistered.submeshCount, unregistered.blockStart, unregistered.blockCount), "topology vertices without a mapping"),
                         })
                {
        ...
        {
            using (var storage = new VpCpuGeometryStorage(16, 16, 4, 8, 16, Allocator.Persistent))
            {
                var table = new VpGeometryReferenceTable(storage, 1, 2);
                VpStoredGeometry stored = Append(storage, Quad());
                VpGeometryReference geometry = Register(table, stored);
