...
{
    public class PngJsonCapturePublicationArtifactPublisherReceiptContractTests
    {
        ...
        private static PngJsonCapturePublicationArtifactPublishReceipt ForgeReceipt(
            IPngJsonCapturePublicationArtifactPublisher issuedBy,
            PngJsonCapturePublicationArtifactPublishOperation operation,
            PngJsonCapturePublicationArtifactRecoveryActionPlan.ValidationToken token)
        {
            PngJsonCapturePublicationArtifactPublishReceipt receipt = (PngJsonCapturePublicationArtifactPublishReceipt)FormatterServices.GetUninitializedObject(
        ...
        public void Factory_NullOperation_Rejected()
        {
            ...
            ArgumentNullException ex = Assert.Throws<ArgumentNullException>(
                () => PngJsonCapturePublicationArtifactPublishReceipt.Create(new FakePublisher(), null, token));
        ...
        public void Factory_StaleToken_Rejected()
        {
            ...
            ArgumentException ex = Assert.Throws<ArgumentException>(
                () => PngJsonCapturePublicationArtifactPublishReceipt.Create(new FakePublisher(), publish, token));
            Assert.That(ex.ParamName, Is.EqualTo("operation"));
        }

        [Test]
        public void Factory_ForgedOperation_Rejected()
        {
            PngJsonCapturePublicationArtifactRecoveryActionPlan plan = BuildPublishPngPlan(
            ...
            ArgumentException ex = Assert.Throws<ArgumentException>(
                () => PngJsonCapturePublicationArtifactPublishReceipt.Create(
                    new FakePublisher(), ForgeOperation(plan, 99, pathSet), token));
            Assert.That(ex.ParamName, Is.EqualTo("operation"));
        }
        ...
        public void IsIssuedFor_SameValueDifferentReferenceOperation_Rejected()
        {
            ...
            Assert.That(receipt.IsIssuedFor(publisher, first, token), Is.True);
            Assert.That(receipt.IsIssuedFor(publisher, second, token), Is.False);
        }
        ...
        public void IsIssuedFor_ReissuedTokenForSamePlan_Rejected()
        {
            ...
            Assert.That(ReferenceEquals(first, second), Is.False);
            Assert.That(receipt.IsIssuedFor(publisher, publish, first), Is.True);
            Assert.That(receipt.IsIssuedFor(publisher, publish, second), Is.False);
        }
        ...
        public void IsIssuedFor_ForeignBackend_Rejected()
        {
            ...
            Assert.That(receipt.IsIssuedFor(publisher, publish, token), Is.True);
            Assert.That(receipt.IsIssuedFor(foreign, publish, token), Is.False);
        }
        ...
        public void Receipt_OwnerRelease_IsValidFalse()
        {
            ...
            Assert.That(receipt.IsIssuedFor(publisher, publish, token), Is.False);
        }
        ...
        public void Receipt_OperationFieldCorruption_IsValidFalse_NoException()
        {
            ...
            // Null operation.
            Assert.That(ForgeReceipt(publisher, null, token).IsValid, Is.False);
            Assert.That(ForgeReceipt(publisher, null, token).IsIssuedFor(publisher, null, token), Is.False);

            // Corrupted operation internals.
            Assert.That(ForgeReceipt(publisher, ForgeOperation(null, 0, pathSet), token).IsValid, Is.False);
            Assert.That(ForgeReceipt(publisher, ForgeOperation(plan, 99, pathSet), token).IsValid, Is.False);
            Assert.That(ForgeReceipt(publisher, ForgeOperation(plan, 0, null), token).IsValid, Is.False);

            // Null token.
            Assert.That(ForgeReceipt(publisher, valid.Operation, null).IsValid, Is.False);
        }

        [Test]
        public void Receipt_PlanDecisionAuthorityCorruption_IsValidFalse()
        {
            PngJsonCapturePublicationArtifactRecoveryActionPlan plan = BuildPublishPngPlan(out _, out _);
            PngJsonCapturePublicationArtifactRecoveryActionPlan.ValidationToken token = plan.AcquireValidationToken();
            PngJsonCapturePublicationArtifactPublishOperation publish =
            ...
            // Substitute the plan's decision: the token's binding must fail closed.
            PngJsonCapturePublicationArtifactRecoveryDecision other = BuildPublishPngPlan(out _, out _).Decision;
        ...
        [Test]
        public void Receipt_StepSubstitutionAfterToken_IsValidFalse()
        {
            PngJsonCapturePublicationArtifactRecoveryActionPlan plan = BuildPublishPngPlan(out _, out _);
            PngJsonCapturePublicationArtifactRecoveryActionPlan.ValidationToken token = plan.AcquireValidationToken();
            PngJsonCapturePublicationArtifactPublishOperation publish =
        ...
        {
            Type type = typeof(PngJsonCapturePublicationArtifactPublishReceipt);
            ...
            ConstructorInfo ctor = type.GetConstructor(
                ...
                new[]
                {
                    typeof(IPngJsonCapturePublicationArtifactPublisher),
                    typeof(PngJsonCapturePublicationArtifactPublishOperation),
        ...
        {
            FieldInfo[] fields = typeof(PngJsonCapturePublicationArtifactPublishReceipt).GetFields(
                BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);

            Assert.That(fields.Length, Is.EqualTo(3));
            Assert.That(fields, Has.Exactly(1).Matches<FieldInfo>(f => f.FieldType == typeof(IPngJsonCapturePublicationArtifactPublisher)));
            ...
            Assert.That(typeof(PngJsonCapturePublicationArtifactPublishReceipt).GetFields(
        ...
        {
            string receiptSource = ReadSource("Assets/Zantetsu/Runtime/Observability/PngJsonCapturePublicationArtifactPublishReceipt.cs");
            string interfaceSource = ReadSource("Assets/Zantetsu/Runtime/Observability/IPngJsonCapturePublicationArtifactPublisher.cs");
            ...
            // The receipt must never re-issue a token or re-validate the whole
            // plan; it validates index-locally with the held token only.
            ...
            Assert.That(receiptSource, Does.Contain("IsValidIndexLocal"));
            Assert.That(receiptSource, Does.Not.Contain("IsValidWithToken"));
        }

        // ---- Scale ----

        [Test]
        public void ThousandPublishReceipts_SingleToken_IndexLocalPerStep()
        {
